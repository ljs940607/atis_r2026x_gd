...
if(blockRoute!=0){
	...
	if(isSubRoute){
		boolean isContentModified = com.matrixone.apps.common.Route.isRouteContentModified(context, sRouteId);
...
<%
            ...
            String modifyContentErrMessage = ComponentsUtil.i18nStringNow("emxComponents.Route.ModifyContent.Resume.ErrorMsg", request.getHeader("Accept-Language"));
			if(ex.getMessage().contains(errorMessage)){
	  			session.setAttribute("error.message",errorMessage);
			}else if(ex.getMessage().contains(modifyContentErrMessage)) {
     			 session.setAttribute("error.message",ComponentsUtil.i18nStringNow("emxComponents.Route.ModifyContent.Resume.ErrorMsg", request.getHeader("Accept-Language")));
			}else if(!clientTaskMessagesExists) {
       			 session.setAttribute("error.message",ComponentsUtil.i18nStringNow("emxComponents.MyRouteSummary.ErrorMessage", request.getHeader("Accept-Language")));
			}
       }
    }
  }
