...
<%
	...
	String scopeType = "";
	String scopeId = "";
	String candidateAssignee = "";
	String currentAssigneeType = "";
	String restrictUserGroup = "";
	String selectUsersOrgGroupName = "";
	System.out.println("selectUsersOrgGroupName::" + selectUsersOrgGroupName);
	
	if(UIUtil.isNotNullAndNotEmpty(objectId)){
		StringList selectList = new StringList();
		selectList.add("type");
		selectList.add("from["+DomainObject.RELATIONSHIP_ROUTE_TASK+"].to.to["+DomainObject.RELATIONSHIP_ROUTE_SCOPE+"].from.type");
		selectList.add("from["+DomainObject.RELATIONSHIP_ROUTE_TASK+"].to.to["+DomainObject.RELATIONSHIP_ROUTE_SCOPE+"].from.id");
		...
		scopeType = (String) objectInfo.get("from["+DomainObject.RELATIONSHIP_ROUTE_TASK+"].to.to["+DomainObject.RELATIONSHIP_ROUTE_SCOPE+"].from.type");
		scopeId = (String) objectInfo.get("from["+DomainObject.RELATIONSHIP_ROUTE_TASK+"].to.to["+DomainObject.RELATIONSHIP_ROUTE_SCOPE+"].from.id");
		candidateAssignee = (String) objectInfo.get("from["+DomainObject.RELATIONSHIP_CANDIDATE_ASSIGNEE+"].to.name");
		currentAssigneeType = (String) objectInfo.get(DomainObject.SELECT_TASK_ASIGNEE_TYPE);
		restrictUserGroup = FrameworkUtil.getRestrictUserGroupValue(context);	
		...
		if(UIUtil.isNotNullAndNotEmpty(routeNodeId)){
			...
			selectUsersOrgGroupName = routeNodeObj.getAttributeValue(context, DomainObject.SELECT_USER_ORG_GROUP_NAME);
		}
...
//XSSOK
 var msg1 = "<%=msg1%>";
 var candidateAssignee = "<%=candidateAssignee%>";
 var currentAssigneeType = "<%=currentAssigneeType%>";
 var restrictUserGroup = "<%=restrictUserGroup%>";
 var msg2 = "<%=msg2%>";
 var msg3 = "<%=msg3%>";
 var msg4 = "<%=msg4%>";
 ...
 var wvScopeType = "<%=scopeType%>";
 var wvScopeId = "<%=scopeId%>";
  ...
  function routeTemplateEditScopeClearAll()
{
  ...
  function checkDefaultAssigneeSelectionOnLoad()
  {
	...
	else if(document.editDataForm.AssigneeSelection[0]){
  ...
  //Validating Name Field in Location Page
	...
	function validateSelector()
	{
  	...
  	function inboxTaskChangeAssigneeChooser() {
		...
		{
		if(document.editDataForm.AssigneeSelection.checked){
			if(restrictUserGroup == "true" && candidateAssignee != null &&  candidateAssignee != "" &&  candidateAssignee != "undefined" && currentAssigneeType == 'Person' ){
				var url = "../common/emxFullSearch.jsp?field=TYPES=type_Person:CURRENT=policy_Person.state_Active:USERROLE="+candidateAssignee+"&showInitialResults=false&txtType=type_Person&mandatorySearchParam=txtType&table=AEFGeneralSearchResults&form=AEFSearchPersonForm&mode=Chooser&selection=single&fieldNameActual=NewAssignee&fieldNameDisplay=NewAssigneeDisplay&fieldNameOID=NewAssigneeOID&submitURL=../common/AEFSearchUtil.jsp&HelpMarker=emxhelpsearch&suiteKey=Components";	
			}else {
		...
		}else if(document.editDataForm.AssigneeSelection[0].checked) {
			if(document.editDataForm.AssigneeSelection[0].value == "Person") {
				var objectId = findFrame(getTopWindow(), "slideInFrame").document.getElementsByName("objectId")[0].value;
				if ("Workspace Vault" == wvScopeType || "Workspace" == wvScopeType) {
					var url = "../common/emxFullSearch.jsp?fieldQueryProgram=emxRoute:getChangeAssigneeRefinements&showInitialResults=false&txtType=type_Person&contextObjectId="+ objectId+ "&mandatorySearchParam=txtType&table=AEFGeneralSearchResults&form=AEFSearchPersonForm&mode=Chooser&selection=single&fieldNameActual=NewAssignee&fieldNameDisplay=NewAssigneeDisplay&fieldNameOID=NewAssigneeOID&submitURL=../common/AEFSearchUtil.jsp&HelpMarker=emxhelpsearch&suiteKey=Components&includeOIDprogram=emxRoute:getPersonsIdListInWorkspace&scopeId="+wvScopeId;
				}else {
					var url = "../common/emxFullSearch.jsp?fieldQueryProgram=emxRoute:getChangeAssigneeRefinements&showInitialResults=false&txtType=type_Person&contextObjectId="+objectId+"&mandatorySearchParam=txtType&table=AEFGeneralSearchResults&form=AEFSearchPersonForm&mode=Chooser&selection=single&fieldNameActual=NewAssignee&fieldNameDisplay=NewAssigneeDisplay&fieldNameOID=NewAssigneeOID&submitURL=../common/AEFSearchUtil.jsp&HelpMarker=emxhelpsearch&suiteKey=Components";
				}
			}
  		}
  		if(document.editDataForm.AssigneeSelection[1]) {
  			if(document.editDataForm.AssigneeSelection[1].checked && document.editDataForm.AssigneeSelection[1].value == "User Group") {
  				if(<%=UINavigatorUtil.isCloud(context)%>){
					...
					if("Workspace Vault" == wvScopeType || "Workspace" == wvScopeType){
						var url = "../common/emxFullSearch.jsp?showInitialResults=true&table=AEFUserGroupsChooser&selection=single&form=AEFSearchUserGroupsForm&cmdName=addUserGroups&field=TYPES=type_Group:CURRENT=policy_Person.state_Active&HelpMarker=emxhelpsearch&editLink=false&fieldNameActual=NewAssignee&fieldNameDisplay=NewAssigneeDisplay&suiteKey=Components&submitURL=../common/AEFSearchUtil.jsp&includeOIDprogram=emxRoute:getAllUserGroupsInScope&scopeId="+wvScopeId;
					}else{
  					var url = "../common/emxFullSearch.jsp?showInitialResults=true&table=AEFUserGroupsChooser&selection=single&form=AEFSearchUserGroupsForm&cmdName=addUserGroups&field=TYPES=type_Group:CURRENT=policy_Person.state_Active&HelpMarker=emxhelpsearch&editLink=false&fieldNameActual=NewAssignee&fieldNameDisplay=NewAssigneeDisplay&suiteKey=Components&submitURL=../common/AEFSearchUtil.jsp";
					}
  	  			}
