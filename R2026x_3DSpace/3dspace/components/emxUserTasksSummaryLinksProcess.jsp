...
<%
	...
	for(int i=0; i<mlist.size(); i++){
		Map map = (Map) mlist.get(i);
		String eSignRequired = (String) map.get(selectRequiresEsign);
		if("True".equalsIgnoreCase(eSignRequired)){
    ...
    String eSignRecordsArry="";
    if(UIUtil.isNotNullAndNotEmpty(isFDAEnabled) && isFDAEnabled.equals("true")){
		eSignRecordsArry=emxGetParameter(request, "eSignRecordId");
					...
					if(UIUtil.isNotNullAndNotEmpty(requiresEsign) && requiresEsign.equalsIgnoreCase("true") && taskRouteAction.equals("Approve")) 
					{
						...
						if(eSignRecords!= null){
							for(int i =0;i<eSignRecords.length;i++){
								String[] eSignObjRefPairMap = eSignRecords[i].split(":");
								if(eSignTaskId.equals(eSignObjRefPairMap[0])){
									eSignList.add(eSignObjRefPairMap[1]+"@"+eSignTaskId+"@"+sAction);
								}
