...
<%
  ...
  if (oids != null) {
    try
    {
	  ...
	  boolean isToProceed = true;
	  if(UINavigatorUtil.isCloud(context)) {
		StringList availableInterfaces = documentVC.getInfoList(context, "interface");		
		if(availableInterfaces.contains("DocumentCreatedFromWebEditor") || availableInterfaces.contains("DocumentOpenedInWebEditor")) {
			session.setAttribute("error.message" , EnoviaResourceBundle.getProperty(context,"emxComponentsStringResource", context.getLocale(),"emxComponents.Document.BlockActionAsWebEditionIsOn"));
			isToProceed = false;
		}
	  }
	  if(isToProceed) {			
