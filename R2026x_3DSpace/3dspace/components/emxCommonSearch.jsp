...
<%!
 ...
 <%
  try
  {
    ...
    if (strMode.equals(Search.ADD_EXISTING)) {
        ...
        params.append("&header=emxCommonSearch.Replace");
          ...
          "true".equalsIgnoreCase(collection)) {
        	...
        	session.setAttribute("CollectionName", objectName);
      	}
	...
	session.setAttribute("params",strParams);

    List menuCommandList = Search.getMenuCommandList(application, request, context, strMenuName, (Vector)session.getAttribute("emxRoleList"), params.toString());
    ...
    session.setAttribute(MENU_COMMAND_LIST, menuCommandList);
  	...
  	session.setAttribute("searchHREF",strHREF);
