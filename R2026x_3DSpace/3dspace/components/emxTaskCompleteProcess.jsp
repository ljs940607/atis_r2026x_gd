            ...
            if ("Yes".equalsIgnoreCase(reviewTask)){
                    ...
                    try{
							...
							if(UIUtil.isNotNullAndNotEmpty(requiresESign) && requiresESign.equals("True")){   //added to update history if ESign Authentication is enabled.
								...
								updateESignApplyRecord(context,eSignRecordId,"Review",eSignReviewTaskId,taskLatestRevision,taskStatus,request);
							}
							
							
                    }catch(Exception ex){
                            session.setAttribute("error.message",ex);
                    }
				...
				if(UIUtil.isNotNullAndNotEmpty(requiresESign) && "True".equals(requiresESign) && taskState.equalsIgnoreCase(DomainObject.STATE_INBOX_TASK_COMPLETE) && UIUtil.isNotNullAndNotEmpty(eSignRecordId)){
					...
					String taskLatestRevision=(String)taskInfoMap.get(DomainConstants.SELECT_REVISION);
					updateESignApplyRecord(context,eSignRecordId,"Complete",eSignTaskId,taskLatestRevision,taskStatus,request);
...
if(getTopWindow().getWindowOpener() || (getTopWindow() && <%="true".equals(fromFDA)%> ) )
{
    ...
    if(getTopWindow().opener && getTopWindow().opener.getTopWindow() && getTopWindow().opener.getTopWindow().RefreshHeader){
		getTopWindow().opener.getTopWindow().deletePageCache();
		getTopWindow().opener.getTopWindow().RefreshHeader();
	}

    if(getTopWindow().opener && getTopWindow().opener.getTopWindow() && getTopWindow().opener.getTopWindow().opener
			&& getTopWindow().opener.getTopWindow().opener.getTopWindow() 
...
public static void updateESignApplyRecord(Context context,String eSignRecordId,String completeReviewTask,String eSignTaskId,String taskLatestRevision, String actionType,HttpServletRequest request){
	try{
		...
		if(UIUtil.isNotNullAndNotEmpty(actionType) &&  actionType!= "" && "Reject".equals(actionType)) {
		...
		updateESignRecord.invoke(eSignRecordUtil, context, eSignRecordId, context.getUser(), actionTakenString, eSignTaskId, "3DSpace", "/v1/resources/task/", "ESignConfigRoute", null, actionType, request);
