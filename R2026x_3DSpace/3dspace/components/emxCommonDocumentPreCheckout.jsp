...
<%
	...
	selects.add(CommonDocument.SELECT_ACTIVE_FILE_VERSION_ID);
	if(UIUtil.isNotNullAndNotEmpty(contextObjectId) && "checkout".equalsIgnoreCase(contextObjectAction)){		
		...
		versionIds = (StringList)objectMap.get(CommonDocument.SELECT_ACTIVE_FILE_VERSION_ID);
	}	
...
<%	
	...
	//web edition check on cloud environment only. If web edition is ON then add error message
	if(UINavigatorUtil.isCloud(context)) 
	{
		//case from version object from table action button 
		if(!UIUtil.isNullOrEmpty(versionId) && !"undefined".equals(versionId))
			isWebEditionSessionON(context, versionId, emxCommonDocumentCheckoutData);
		//case from Document major object from table action button 
		else if(!versionIds.isEmpty() && (UIUtil.isNullOrEmpty(versionId) || versionId.equals("undefined")))
		{
			Iterator itr = versionIds.iterator();
			while(itr.hasNext()) {
				String versionID = (String) itr.next();
				if(isWebEditionSessionON(context, versionID, emxCommonDocumentCheckoutData))
					break;
			}
		}
		//case from version object from action menu option
		else {
			String[] tableRowIds = (String[]) emxCommonDocumentCheckoutData.get("emxTableRowId");
			if(tableRowIds != null && tableRowIds.length > 0) {
				Map objectMap = UIUtil.parseRelAndObjectIds(context, tableRowIds,false);
				tableRowIds = (String[])objectMap.get("objectIds");
				for(int index = 0; index < tableRowIds.length; index++)
					if(isWebEditionSessionON(context, tableRowIds[index], emxCommonDocumentCheckoutData))
						break;
			}
		}
	}
...
var msfByPass=false;
			   ...
			   msfByPass=true;
				if(MSFDocumentClient.isConnectedWithMSF() === 'true') {	
					...
					%>
				if("<%=XSSUtil.encodeForJavaScript(context, fileVersionId)%>" === "null"){
						MSFDocumentClient.MSFCallCheckout("<%= XSSUtil.encodeForJavaScript(context, action)%>", "<%= XSSUtil.encodeForJavaScript(context, objectIds[0])%>", null, "", "", "", "", "");
				}
				else{
					MSFDocumentClient.MSFCallCheckout("<%= XSSUtil.encodeForJavaScript(context, action)%>","<%= XSSUtil.encodeForJavaScript(context, objectIds[0])%>", null, "", "","<%=XSSUtil.encodeForJavaScript(context, fileVersionId)%>", "", "");
				}	
						return;
					<%}%>
					}else{
						if(<%=checkoutDirectoryEmpty%>){ 
							 ...
							 getTopWindow().showModalDialog("../components/emxCommonDocumentCheckout.jsp?isRequiredPath="+"<%=XSSUtil.encodeForJavaScript(context, isRequiredPath)%>"+"&isExpandFromDEC="+"<%=XSSUtil.encodeForJavaScript(context, isExpandFromDEC)%>", 500, 500, true);
						    ...
						    // IR-158505: In case of checkout from pop (as for archive functionality), the showModalDialog() needs to be invoked on getTopWindow().getWindowOpener().getTopWindow().
						        if(<%=XSSUtil.encodeForJavaScript(context,isAppPreProcess)%>){
							         document.location.href="../components/emxCommonDocumentCheckout.jsp?isRequiredPath="+"<%=XSSUtil.encodeForJavaScript(context, isRequiredPath)%>"+"&isExpandFromDEC="+"<%=XSSUtil.encodeForJavaScript(context, isExpandFromDEC)%>";
									...
									getTopWindow().showModalDialog("../components/emxCommonDocumentCheckout.jsp?isRequiredPath="+"<%=XSSUtil.encodeForJavaScript(context, isRequiredPath)%>"+"&isExpandFromDEC="+"<%=XSSUtil.encodeForJavaScript(context, isExpandFromDEC)%>", 730, 450, true);
								}else if(getTopWindow().getWindowOpener() && getTopWindow().getWindowOpener().getTopWindow()){
								  getTopWindow().getWindowOpener().getTopWindow().showModalDialog("../components/emxCommonDocumentCheckout.jsp?isRequiredPath="+"<%=XSSUtil.encodeForJavaScript(context, isRequiredPath)%>"+"&isExpandFromDEC="+"<%=XSSUtil.encodeForJavaScript(context, isExpandFromDEC)%>", 730, 450, true);
							}else {
							  getTopWindow().showModalDialog("../components/emxCommonDocumentCheckout.jsp?isRequiredPath="+"<%=XSSUtil.encodeForJavaScript(context, isRequiredPath)%>"+"&isExpandFromDEC="+"<%=XSSUtil.encodeForJavaScript(context, isExpandFromDEC)%>", 730, 450, true);
							}
						  }  else {
...
public static boolean isWebEditionSessionON(Context context, String versionId, Map emxCommonDocumentCheckoutData) throws Exception {
	DomainObject versionObject = new DomainObject(versionId);
	StringList availableInterfaces = versionObject.getInfoList(context, "interface");
	if(availableInterfaces.contains("DocumentCreatedFromWebEditor") || availableInterfaces.contains("DocumentOpenedInWebEditor")) {
		emxCommonDocumentCheckoutData.put("error.message", EnoviaResourceBundle.getProperty(context,"emxComponentsStringResource",context.getLocale(),"emxComponents.Document.BlockActionAsWebEditionIsOn"));
		return true;
	}
	return false;
}
