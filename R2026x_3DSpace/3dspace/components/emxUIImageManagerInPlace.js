	    ...
	    var manager = {
	    		...
	    		onImageCollectionChanged: function(initialImage){
			    	...
			    	if(that.imageCollection.length > 0){
			    		...
			    		if(typeof initialImage === "string" || typeof initialImage === null) { //must be a file name
	    		...
	    		init: function (objectId) {
				    ...
				    jQuery(".modal-content", that.dialogElement).resizable({
				    	...
				    	resize: function( event, ui ) {
				    	    ...
				    	    if(isSouth){
				    	    	var ratio = jQuery('.img-viewer-outer', that.dialogElement).width()/jQuery('.img-viewer-outer', that.dialogElement).height() || 0;
