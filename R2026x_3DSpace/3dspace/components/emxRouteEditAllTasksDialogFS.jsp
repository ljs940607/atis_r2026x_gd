...
<%
  ...
  String ATTRIBUTE_TASKEDIT_SETTING = PropertyUtil.getSchemaProperty(context,"attribute_TaskEditSetting");
  Route route = (Route)DomainObject.newInstance(context,routeId);
  StringList selects = new StringList();
  selects.add(route.SELECT_ROUTE_TEMPLATE_ID);
  selects.add(DomainConstants.SELECT_CURRENT);
  selects.add("attribute["+ATTRIBUTE_TASKEDIT_SETTING+"]");
  
  Map roundInfo = route.getInfo(context,selects);
  String templateId=(String)roundInfo.get(route.SELECT_ROUTE_TEMPLATE_ID);
  String routeState = (String)roundInfo.get(DomainConstants.SELECT_CURRENT);
  if ((templateId != null) && (!templateId.equals("")))
  {
	strTaskEditSetting   = getTaskSetting(context,templateId);
  }
  if (strTaskEditSetting == null && !DomainConstants.STATE_ROUTE_DEFINE.equalsIgnoreCase(routeState)){
	  strTaskEditSetting = (String)roundInfo.get("attribute["+ATTRIBUTE_TASKEDIT_SETTING+"]");
  }
