...
<%
    ...
    if("Indexed".equals(systemQueryType) && "Indexed".equals(libraryQueryType)){
        ...
        sbSubmitActionURL.append("&selection=single");
		...
		sbSubmitActionURL.append("&submitURL=../documentcentral/emxMultipleClassificationEndItemsSearchPostProcess.jsp");
...
<% if(showFullTextsearch){%>
    ...
    showModalDialog("<xss:encodeForJavaScript><%=sbSubmitActionURL.toString()%></xss:encodeForJavaScript>", 700, 500, true);
    </script>
<%}else{ %>
    <script language="javascript">
    showModalDialog("<xss:encodeForJavaScript><%=strSubmitActionURL%></xss:encodeForJavaScript>", 700, 500, true);
    </script>
<%} %>
