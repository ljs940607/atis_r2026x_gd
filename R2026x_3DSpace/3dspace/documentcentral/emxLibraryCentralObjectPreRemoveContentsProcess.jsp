...
<%
  ...
  String strStandardConfirmMessage = EnoviaResourceBundle.getProperty(context,LC_STRING_RESOURCE, new Locale(sLanguage),"emxLibraryCentral.Remove.ConfirmMsg");
    ...
    {
	      String[] arrayOfIds = childIds[i].split("\\|");
        String objectId = arrayOfIds[1];
        childId += objectId +",";
    }
%>
	...
	<input type="hidden" name= "removeClassified" value="<%=XSSUtil.encodeForHTMLAttribute(context,isRemoveClassified)%>" />
...
<script language="javascript">
     var isToBeContinued = true;
       ...
       if (isToBeContinued)
       {
          <%-- listHiddenFrame.location.href = "emxLibraryCentralObjectRemoveContentsProcess.jsp?objectId=<xss:encodeForJavaScript><%=parentId%></xss:encodeForJavaScript>&removeClassified=<xss:encodeForJavaScript><%=isRemoveClassified%></xss:encodeForJavaScript></xss:encodeForJavaScript>&childIds=<xss:encodeForJavaScript><%=childId%></xss:encodeForJavaScript>&ENO_CSRF_TOKEN=<xss:encodeForJavaScript><%=csrfTokenValue%></xss:encodeForJavaScript>"; --%>
