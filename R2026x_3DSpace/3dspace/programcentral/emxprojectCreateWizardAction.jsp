          ...
          // Logid added by Pavan on 28-Feb-2006 to ensure already related projects are not connected and eliminate the Transaction exception seen on the browser
            ...
            //IR-1340365:START
              StringList slSelect = new StringList(8);
        		...
        		slSelect.addElement(ProgramCentralConstants.SELECT_ATTRIBUTE_TITLE);
        		slSelect.addElement(ProgramCentralConstants.SELECT_KINDOF_PROJECT_SPACE);
        		slSelect.addElement(ProgramCentralConstants.SELECT_KINDOF_EXPERIMENT_PROJECT);
        		slSelect.addElement(ProgramCentralConstants.SELECT_KINDOF_PROJECT_BASELINE);
        		
        		MapList templateIdsType = DomainObject.getInfo(context, templateIds, slSelect);
  			 
  			 boolean isObjectNotAllowed= false;
  			 String objNotAddedAsRelatedProject = "";
  			 for(int j=0;j<templateIdsType.size();j++){
  				 Map <String,String> obj = (Map)templateIdsType.get(j);
  				 boolean isObjTypeProjectSpace = "true".equalsIgnoreCase((String)obj.get(ProgramCentralConstants.SELECT_KINDOF_PROJECT_SPACE));
  				 boolean isObjTypeExperiment = "true".equalsIgnoreCase((String)obj.get(ProgramCentralConstants.SELECT_KINDOF_EXPERIMENT_PROJECT));
  				 boolean isObjTypeBaseline = "true".equalsIgnoreCase((String)obj.get(ProgramCentralConstants.SELECT_KINDOF_PROJECT_BASELINE));
  				 String objState = (String)obj.get(DomainObject.SELECT_CURRENT);
  				 String objID = (String)obj.get(DomainObject.SELECT_ID);
  				 if(!alreadyRelatedProjects.contains(objID)) {
  					 if(!isObjTypeProjectSpace 
  							 ||  (isObjTypeProjectSpace  
  									 && (objState.equals(ProgramCentralConstants.STATE_PROJECT_REVIEW_COMPLETE) 
  									 ||  objState.equals(ProgramCentralConstants.STATE_PROJECT_REVIEW_ARCHIEVE)
  									 ||  objState.equals(ProgramCentralConstants.STATE_PROJECT_SPACE_HOLD_CANCEL_HOLD)
  									 ||  isObjTypeExperiment
  									 ||  isObjTypeBaseline))){
  						 String objName = (!isObjTypeExperiment && !isObjTypeBaseline) ? (String)obj.get(ProgramCentralConstants.SELECT_ATTRIBUTE_TITLE) : (String)obj.get(DomainObject.SELECT_NAME) ;
  						 objNotAddedAsRelatedProject += ((j!=0 && isObjectNotAllowed) ? ", " : "") + objName;			
  						 isObjectNotAllowed = true;
  					 }else{
  			 ...
  			 if(isObjectNotAllowed){
  				 System.out.println("isObjectNotAllowed :: " + isObjectNotAllowed);
  				 %>
  				 <script language="javascript" type="text/javaScript">
  				 alert("<framework:i18nScript localize="i18nId">emxProgramCentral.Project.CannotAddAsRelatedProjects</framework:i18nScript>\n<%=objNotAddedAsRelatedProject%>");		
  				 </script>
