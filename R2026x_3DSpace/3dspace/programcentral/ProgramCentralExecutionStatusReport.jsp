...
<%
	...
	session.setAttribute(sOID+"_store", report);
...
<%						
			...
			var url = "../programcentral/emxProgramCentralUtil.jsp?mode=updateSessionWithSummaryTasks&random=" + new Date().getTime()+ "&objectId=<%=sOID%>";
			...
			var linkClick = function(method, header, sOID) {
				
				var url = "../programcentral/emxProjectManagementUtil.jsp?mode=getDueTasks&method=" + method + "&random=" + new Date().getTime() + "&objectId=" + sOID;
				...
				if(header.indexOf('<%=report.DELIVERABLES%>') > -1){
								...
								events: {
					                click: function (event) {
					                	if(this.series.name=='<%=report.STATUS_PENDING%>')//for pending task
					                		linkClick ('getTopLevelPendingTask','<%=report.GANTT_HEADER_TOPLEVEL_PENDING_TASKS%>' , '<%=sOID%>');
					                	else if(this.series.name=='<%=report.STATUS_OVERDUE%>')//overduetask
					                		linkClick ('getTopLevelOverdueTask','<%=report.GANTT_HEADER_TOPLEVEL_OVERDUE_TASKS%>' , '<%=sOID%>');	
					                	else if(this.series.name=='<%=report.STATUS_LATE_START%>')//late start
					                		linkClick ('getTopLevelLateStartTask','<%=report.GANTT_HEADER_TOPLEVEL_LATESTART_TASKS%>' , '<%=sOID%>');
					                	else if(this.series.name=='<%=report.STATUS_COMPLETED%>')//completed
					                		linkClick ('getTopLevelCompleteTask','<%=report.GANTT_HEADER_TOPLEVEL_COMPLETED_TASKS%>' , '<%=sOID%>');              		
					              	  }
									}
								}
		                },
						...
						series: {
							...
							events: {
				                click: function (event) {
				                	...
				                	linkClick ('getAllPendingTask','<%=report.GANTT_HEADER_All_PENDING_TASKS%>' , '<%=sOID%>');
				                	else if(this.x==1)//overduetask
				                		linkClick ('getAllOverdueTask','<%=report.GANTT_HEADER_All_OVERDUE_TASKS%>' , '<%=sOID%>');	
				                	else if(this.x==2)//late start
				                		linkClick ('getAllLateStartTask','<%=report.GANTT_HEADER_All_LATESTART_TASKS%>' , '<%=sOID%>');
				                	else if(this.x==3)//compeleted
				                		linkClick ('getAllCompleteTask','<%=report.GANTT_HEADER_All_COMPLETED_TASKS%>' , '<%=sOID%>');
						...
						series: {
							...
							point:{
								events: {
					                click: function (event) {
					                	if(this.x==0)//for pending task
					                	linkClick ('getPendingDeliverables','<%=report.GANTT_HEADER_PENDING_TASKS_WITH_DELIVERABLES%>' , '<%=sOID%>');
					                	else if(this.x==1)//overduetask
					                		linkClick ('getOverdueDeliverables','<%=report.GANTT_HEADER_OVERDUE_TASKS_WITH_DELIVERABLES%>' , '<%=sOID%>');	
					                	else if(this.x==2)//late start
					                		linkClick ('getLateStartDeliverables','<%=report.GANTT_HEADER_LATESTART_TASKS_WITH_DELIVERABLES%>' , '<%=sOID%>');
					                	else if(this.x==3)//compeleted
					                		linkClick ('getCompletedDeliverables','<%=report.GANTT_HEADER_COMPLETED_TASKS_WITH_DELIVERABLES%>' , '<%=sOID%>');
						...
						series: {
							...
							point:{
								events: {
					                click: function (event) {
					                	if(this.x==0)//for pending task
					                	linkClick ('getPendingCriticalTask','<%=report.GANTT_HEADER_CRITICAL_PENDING_TASKS%>' , '<%=sOID%>');
					                	else if(this.x==1)//overduetask
					                		linkClick ('getOverdueCriticalTask','<%=report.GANTT_HEADER_CRITICAL_OVERDUE_TASKS%>' , '<%=sOID%>');	
					                	else if(this.x==2)//late start
					                		linkClick ('getLateStartCriticalTask','<%=report.GANTT_HEADER_CRITICAL_LATESTART_TASKS%>' , '<%=sOID%>');
					                	else if(this.x==3)//compeleted
					                		linkClick ('getCompletedCriticalTask','<%=report.GANTT_HEADER_CRITICAL_COMPLETED_TASKS%>' , '<%=sOID%>');
						...
						<td>
							...
							<div class="info" id="divInfoTasks"  >
								<table width="100%">
									<tr>
										<td width="5px">&nbsp;</td>
										<% if (wbsLeafTasks[2][0] == 0){%>
										<td width="25%" class="link" style="text-align:left"><b><%=wbsLeafTasks[2][0]%></b><%=report.DUE_THIS_WEEK%></td>
										<% }else {%>
										<td width="25%" class="link" style="text-align:left"><b><%=wbsLeafTasks[2][0]%></b>   <a href="javascript:linkClick('getTasksDueThisWeek' 	, '<%=report.GANTT_HEADER_TASKS_DUE_THIS_WEEK%>', '<%=sOID%>')" 	target=""><%=report.DUE_THIS_WEEK%></a></td>
										<% }
										if (wbsLeafTasks[2][1] == 0){%>
										<td width="25%" class="link" style="text-align:center"><b><%=wbsLeafTasks[2][1]%></b><%=report.DUE_THIS_MONTH%></td>
										<% }else {%>										
										<td width="25%" class="link" style="text-align:center"><b><%=wbsLeafTasks[2][1]%></b> <a href="javascript:linkClick('getTasksDueThisMonth'	, '<%=report.GANTT_HEADER_TASKS_DUE_THIS_MONTH%>', '<%=sOID%>')" target=""><%=report.DUE_THIS_MONTH%></a></td>
										<% }
										if (wbsLeafTasks[2][2] == 0){%>
										<td width="25%" class="link" style="text-align:center"><b><%=wbsLeafTasks[2][2]%></b><%=report.SOON%></td>
										<% }else {%>
										<td width="25%" class="link" style="text-align:center"><b><%=wbsLeafTasks[2][2]%></b> <a href="javascript:linkClick('getTasksDueSoon'		, '<%=report.GANTT_HEADER_TASKS_SOON%>', '<%=sOID%>')" 			target=""><%=report.SOON%></a></td>
										<% }
										if (wbsLeafTasks[2][3] == 0){%>
										<td width="25%" class="link" style="text-align:right"><b><%=wbsLeafTasks[2][3]%></b><%=report.OVERDUE%></td>
										<% }else {%>
										<td width="25%" class="link" style="text-align:right"><b><%=wbsLeafTasks[2][3]%></b>  <a href="javascript:linkClick('getTasksOverdue'		, '<%=report.GANTT_HEADER_TASKS_OVERDUE%>', '<%=sOID%>')" 		target=""><%=report.OVERDUE%></a></td>
										<% }%>
										<td width="5px">&nbsp;</td>
									</tr>
								</table>
						...
						<td>
							...
							<div class="info" id="divInfoDeliverables">
								<table width="100%">
									<tr>
										<td width="5px">&nbsp;</td>	
										<% if (wbsDeliverableTasks[2][0] == 0){%>
										<td width="25%" class="link" style="text-align:left"><b><%=wbsDeliverableTasks[2][0]%></b><%=report.DUE_THIS_WEEK%></td>
										<% }else {%>
										<td width="25%" class="link" style="text-align:left"><b><%=wbsDeliverableTasks[2][0]%> </b>  <a href="javascript:linkClick('getDeliverablesDueThisWeek'	, 	'<%=report.GANTT_HEADER_DELIVERABLES_DUE_THIS_WEEK%>' , '<%=sOID%>')" target=""><%=report.DUE_THIS_WEEK%></a></td>
										<% }
										if (wbsDeliverableTasks[2][1] == 0){%>
										<td width="25%" class="link" style="text-align:center"><b><%=wbsDeliverableTasks[2][0]%></b><%=report.DUE_THIS_MONTH%></td>
										<% }else {%>
										<td width="25%" class="link" style="text-align:center"><b><%=wbsDeliverableTasks[2][1]%></b> <a href="javascript:linkClick('getDeliverablesDueThisMonth', 	'<%=report.GANTT_HEADER_DELIVERABLES_DUE_THIS_MONTH%>', '<%=sOID%>')" target=""><%=report.DUE_THIS_MONTH%></a></td>
										<% }
										if (wbsDeliverableTasks[2][2] == 0){%>
										<td width="25%" class="link" style="text-align:center"><b><%=wbsDeliverableTasks[2][0]%></b><%=report.SOON%></td>
										<% }else {%>
										<td width="25%" class="link" style="text-align:center"><b><%=wbsDeliverableTasks[2][2]%></b> <a href="javascript:linkClick('getDeliverablesDueSoon'		, 	'<%=report.GANTT_HEADER_DELIVERABLES_SOON%>'    , '<%=sOID%>' )" target=""><%=report.SOON%></a></td>										
										<% }
										if (wbsDeliverableTasks[2][3] == 0){%>
										<td width="25%" class="link" style="text-align:right"><b><%=wbsDeliverableTasks[2][0]%></b><%=report.OVERDUE%></td>
										<% }else {%>
										<td width="25%" class="link" style="text-align:right"><b><%=wbsDeliverableTasks[2][3]%></b>  <a href="javascript:linkClick('getDeliverablesOverdue'		,	'<%=report.GANTT_HEADER_DELIVERABLES_OVERDUE%>'  , '<%=sOID%>')" target=""><%=report.OVERDUE%></a></td>									
										<% }%>
										<td width="5px">&nbsp;</td>
									</tr>
								</table>
						...
						<td>
							...
							<div class="info" id="divInfoCriticalTasks">
								<table width="100%">
									<tr>
										<td width="5px">&nbsp;</td>			
										<% if (criticalLeafTasks[2][0] == 0){%>
										<td width="25%" class="link" style="text-align:left"><b><%=criticalLeafTasks[2][0]%></b><%=report.DUE_THIS_WEEK%></td>
										<% }else {%>			
										<td width="25%" class="link" style="text-align:left"><b><%=criticalLeafTasks[2][0]%></b>   <a href="javascript:linkClick('getCriticalTasksDueThisWeek', '<%=report.GANTT_HEADER_CRITICAL_TASKS_DUE_THIS_WEEK%>' , '<%=sOID%>')" target=""><%=report.DUE_THIS_WEEK%></a></td>
										<% }
										if (criticalLeafTasks[2][1] == 0){%>
										<td width="25%" class="link" style="text-align:center"><b><%=criticalLeafTasks[2][1]%></b><%=report.DUE_THIS_MONTH%></td>
										<% }else {%>
										<td width="25%" class="link" style="text-align:center"><b><%=criticalLeafTasks[2][1]%></b> <a href="javascript:linkClick('getCriticalTasksDueThisMonth','<%=report.GANTT_HEADER_CRITICAL_TASKS_DUE_THIS_MONTH%>', '<%=sOID%>')" target=""><%=report.DUE_THIS_MONTH%></a></td>
										<% }
										if (criticalLeafTasks[2][2] == 0){%>
										<td width="25%" class="link" style="text-align:center"><b><%=criticalLeafTasks[2][2]%></b><%=report.SOON%></td>
										<% }else {%>
										<td width="25%" class="link" style="text-align:center"><b><%=criticalLeafTasks[2][2]%></b> <a href="javascript:linkClick('getCriticalTasksDueSoon', 	'<%=report.GANTT_HEADER_CRITICAL_TASKS_SOON%>'     , '<%=sOID%>')" target=""><%=report.SOON%></a></td>
										<% }
										if (criticalLeafTasks[2][3] == 0){%>
										<td width="25%" class="link" style="text-align:right"><b><%=criticalLeafTasks[2][3]%></b><%=report.OVERDUE%></td>
										<% }else {%>
										<td width="25%" class="link" style="text-align:right"><b><%=criticalLeafTasks[2][3]%> </b> <a href="javascript:linkClick('getCriticalTasksOverdue', 	'<%=report.GANTT_HEADER_CRITICAL_TASKS_OVERDUE%>' , '<%=sOID%>' )" target=""><%=report.OVERDUE%></a></td>				
										<% }%>
										<td width="5px">&nbsp;</td>
									</tr>
								</table>
