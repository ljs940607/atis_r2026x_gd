<%--  emxProgramCentralUIFreezePaneValidation.jsp
 ...
 --%>
  ...
  <script src="../webapps/VENCDjquery/latest/dist/jquery.min.js"></script> 
...
<%
	...
	String strI18Day = EnoviaResourceBundle.getProperty(context, "ProgramCentral","emxProgramCentral.DurationUnits.Day", strLanguage);
	String strI18Hour = EnoviaResourceBundle.getProperty(context, "ProgramCentral","emxProgramCentral.DurationUnits.Hour", strLanguage);
	String strI18Week = EnoviaResourceBundle.getProperty(context, "ProgramCentral","emxProgramCentral.DurationUnits.Week", strLanguage);
	String strI18Month = EnoviaResourceBundle.getProperty(context, "ProgramCentral","emxProgramCentral.DurationUnits.Month", strLanguage);
	...
	strI18Day = strI18Day.toLowerCase();
	strI18Hour = strI18Hour.toLowerCase();
	strI18Week = strI18Week.toLowerCase();
	strI18Month = strI18Month.toLowerCase();
	
	strI18Days = strI18Days.toLowerCase();
	strI18Hours = strI18Hours.toLowerCase();
	strI18Weeks = strI18Weeks.toLowerCase();
	strI18Months = strI18Months.toLowerCase();
...
function isParentChildStructure(predecessor, successor, delimiter){
	var predecessorLevel = predecessor.split(delimiter);
	var successorLevel = successor.split(delimiter);
	var predecessorLevelLength =predecessorLevel.length;
	var successorLevelLength = successorLevel.length;
	var isParentChildDependancy = true;
	
	if(predecessorLevelLength === successorLevelLength){
		isParentChildDependancy = false;
	} 
	else{
		var lowerLevelTaskArray = predecessorLevel;
		var higherLevelTaskArray = successorLevel;
		if(predecessorLevelLength > successorLevelLength){
			lowerLevelTaskArray = successorLevel;
			higherLevelTaskArray = predecessorLevel;
		}
		
		for (i = 0; i < lowerLevelTaskArray.length; i++) {
			if(lowerLevelTaskArray[i] === higherLevelTaskArray[i]){
				continue;
			}
			else{
				isParentChildDependancy = false;
	...
	return isParentChildDependancy;
}
...
function checkParentChildDependancy(strEnteredDependancy,cellIdValue,cellRowId){
	...
	if(window && window.dataGridEnabled){
		...
		if(matchedNode !== ""){
				...
				if(isParentChildStructure(cellRowId, parentRowId, ',')){
					isPCDependency = true;
					taskName = "name";
					errorUrl = "../";
				}
		...
		if(enteredLevelValue && enteredLevelValue!="" && currentLevelValue && currentLevelValue!=""){
			...
			if(isParentChildStructure(currentLevelValue, enteredLevelValue, '.')){
				isPCDependency = true;
				taskName = "ID";
			}
		}
		...
		for (i = 0, tableRows; tableRows = table.rows[i]; i++) {
			...
			if(currentCell){
				...
				if(currentIdValue === strEnteredDependancy){
					...
					if(isParentChildStructure(cellRowId, currentRowId, ',')){
						isPCDependency = true;
						taskName = "ID";
					}
				}
...
/* ---------------------------- Validate circular dependency End ------------------------------------------ */
    ...
    if(!isEmpty(dependencyVal))
    {
		...
		if(STR_DEC_SYM == ",")
		{
			var regexp = /^((([a-zA-Z0-9_-]+((\s\w+)+)?:)?\d+:[FSfs]{2}([\+\-]\d+(\,\d+)?\s([DdHhWw]|[Mm][Oo][Nn]))?)(\,((\w+((\s\w+)+)?:)?\d+:[FSfs]{2}([\+\-]\d+(\,\d+)?\s([DdHhWw]|[Mm][Oo][Nn]))?))*)?$/;
		}
		else
		{
        var regexp = /^((([a-zA-Z0-9_-]+((\s\w+)+)?:)?\d+:[FSfs]{2}([\+\-]\d+(\.\d+)?\s([DdHhWw]|[Mm][Oo][Nn]))?)(\,((\w+((\s\w+)+)?:)?\d+:[FSfs]{2}([\+\-]\d+(\.\d+)?\s([DdHhWw]|[Mm][Oo][Nn]))?))*)?$/;
...
/* ---------------------------- Validate circular dependency End ------------------------------------------ */
    ...
    if(!isEmpty(dependencyVal))
    {
		...
		if(STR_DEC_SYM == ",")
		{
			var regexp = /^((\d+:[FSfs]{2}([\+\-]\d+(\,\d+)?\s([DdHhWw]|[Mm][Oo][Nn]))?)(\,(\d+:[FSfs]{2}([\+\-]\d+(\,\d+)?\s([DdHhWw]|[Mm][Oo][Nn]))?))*)?$/;
		}
		else
		{
        	var regexp = /^((\d+:[FSfs]{2}([\+\-]\d+(\.\d+)?\s([DdHhWw]|[Mm][Oo][Nn]))?)(\,(\d+:[FSfs]{2}([\+\-]\d+(\.\d+)?\s([DdHhWw]|[Mm][Oo][Nn]))?))*)?$/;
...
function validateDuration()
{
    ...
    var hour = "<%=strI18Hour%>";
    var week = "<%=strI18Week%>";
    var month = "<%=strI18Month%>";
     
    var days = "<%=strI18Days%>";
    var hours = "<%=strI18Hours%>";
    var weeks = "<%=strI18Weeks%>";
    var months = "<%=strI18Months%>";
      
    if(durationVal.indexOf(day) != -1 || durationVal.indexOf(days) != -1){
    	durationVal = durationVal.replace(days,"d");
    	durationVal = durationVal.replace(day,"d");
    } else if(durationVal.indexOf(hour) != -1 || durationVal.indexOf(hours) != -1) {
        durationVal = durationVal.replace(hours,"h");
        durationVal = durationVal.replace(hour,"h");
    } else if(durationVal.indexOf(week) != -1 || durationVal.indexOf(weeks) != -1) {
        durationVal = durationVal.replace(weeks,"w");
        durationVal = durationVal.replace(week,"w");
    }else if(durationVal.indexOf(month) != -1 || durationVal.indexOf(months) != -1) {
    ...
    if(!durationVal.endsWith(' h') && !durationVal.endsWith(' d')&& !durationVal.endsWith(' w')&& !durationVal.endsWith(' mon')&&!isValidNumeric(durationVal)){
		alert(errmsg);
		flagValid=1;
		return false;
	}
    ...
    if(durationVal != null)
    {
	  ...
	  if((durationUnit==='h' && durationVal >=80000) || (durationUnit==='d' && durationVal >=10000) || (durationUnit==='w' && durationVal >=2000) || (durationUnit==='mon' && durationVal >=500)) {
...
function reloadTaskCompletionTypeRangeValues()
{
    emxEditableTable.reloadCell("TaskCompletionType");
...
function isBadNameCharsSB(){
    ...
    for (var i=0; i < ARR_NAME_BAD_CHARS.length; i++)
	{
		if (ARR_NAME_BAD_CHARS[i] != '' && fieldValue.indexOf(ARR_NAME_BAD_CHARS[i]) > -1){
			...
			//IR-1310512-3DEXPERIENCER2025x
			 // If any bad character is found, show an alert with all the bad characters, not just the ones in fieldValue
            alert("<framework:i18nScript localize='i18nId'>emxProgramCentral.Common.InvalidCharacters</framework:i18nScript>" + "\n" + sBadChars);
            return false;
	...
	//}else{
		...
		// If no bad characters are found, return true
		return true;
	}
