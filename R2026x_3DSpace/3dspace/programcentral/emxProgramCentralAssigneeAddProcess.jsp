...
//  com.matrixone.apps.program.Opportunity opportunity = (com.matrixone.apps.program.Opportunity) DomainObject.newInstance(context, RiskManagement.TYPE_OPPORTUNITY, "PROGRAM");
  ...
  String isAssignedTasksCandidateAdd = emxGetParameter(request, "isAssignedTasksCandidateAdd");
  ...
  if (searchResultList != null){
    ...
    else
    {
      try {
		...
		} else if("true".equalsIgnoreCase(isAssignedTasksCandidateAdd)) {
			boolean includeUserGroupInfo = true;
			boolean includeUserGroupMembers = true;
			assigneesList = task.getAssignees(context, busSelects, relSelects, null,ProgramCentralConstants.RELATIONSHIP_ASSIGNED_TASKS_CANDIDATE,includeUserGroupInfo, includeUserGroupMembers);
		} else {
        ...
        while (resultListItr.hasNext())
        {
          ...
          if (!taskAssigneeMap.containsKey(selectedPerson)){
        	  ...
        	  if(isExperimentTask){
				 ...
				 task.addAssignee(context, selectedPerson, null,null,ProgramCentralConstants.RELATIONSHIP_ASSIGNED_TASKS_CANDIDATE);
			  } else {
...
<html>
  <script language="javascript" type="text/javaScript">//<![CDATA[
	...
	var isAssignedTasksCandidateAdd = "<%=isAssignedTasksCandidateAdd%>";  
			...
			var topFrame = findFrame(getTopWindow(), "PMCWBSDataGridView");
		  	if (topFrame == null || topFrame.location.href=="about:blank") {
		  		...
		  		getTopWindow().getWindowOpener().parent.refreshSBTable(getTopWindow().getWindowOpener().parent.configuredTableName);                       
		  			...
		  			var tableName = topFrame.configuredTableName;
					if (tableName == "PMCWBSAssignmentViewTable" || tableName == "PMCWBSAssignmentAllocationViewTable"){
						getTopWindow().getWindowOpener().parent.refreshSBTable(getTopWindow().getWindowOpener().parent.configuredTableName,null,null,"true"); 
					}
						
		  		}
			}else{
				var href = topFrame.location.href;
				
				
				if (href.includes("PMCWBSAssignmentViewTableENXSBGrid") || href.includes("PMCWBSAllocationViewTableENXSBGrid")){
				topFrame.location.href = topFrame.location.href;
	  		}
		} 
		} 
