/// <amd-module name="DS/SMACXPSimRTScript/StuAnimationPlayer"/>
...
define("DS/SMACXPSimRTScript/StuAnimationPlayer", ["require", "exports", "DS/CXPTypings/CXPEP", "DS/StuCore/StuContext", "DS/StuCore/StuManager", "DS/StuModel/StuAnimationStartedEvent", "DS/StuModel/StuAnimationFinishedEvent", "DS/StuModel/StuAnimationPausedEvent", "DS/StuModel/StuAnimationResumedEvent", "DS/StuModel/StuAnimationStoppedEvent", "DS/StuModel/StuAnimationLoopFinishedEvent", "DS/StuModel/StuAnimationLoopStartedEvent"], function (require, exports, EP, STU, Manager, AnimationStartedEvent, AnimationFinishedEvent, AnimationPausedEvent, AnimationResumedEvent, AnimationStoppedEvent, AnimationLoopFinishedEvent, AnimationLoopStartedEvent) {
    "use strict";
    // ! CODE DUPLICATED FROM XCTCXPKinAnimationModelWeb\CXPKinAnimation.mweb\src\StuAnimationPlayer.ts !
    // to avoid cyclic reference
    //import ProductAnimation = require("DS/SMACXPSimRTScript/StuProductAnimation");
    /**
     * Animation task
     *
     * @private
     * @class AnimationPlayerTask
     * @extends {EP.Task}
     */
    var AnimationPlayerTask = /** @class */ (function (_super) {
        __extends(AnimationPlayerTask, _super);
        function AnimationPlayerTask(player) {
            var _this = _super.call(this) || this;
            _this.player = player;
            return _this;
        }
        AnimationPlayerTask.prototype.onExecute = function (iPlayerCtx) {
            this.player.onExecute(iPlayerCtx);
        };
        AnimationPlayerTask.prototype.onStop = function () {
            var _this = this;
            // reset played animations
            this.player._registeredAnimations.forEach(function (simAnimationState) {
                var simAnimation = simAnimationState.simAnim;
                if (simAnimation._wrapper !== null && simAnimation._wrapper !== undefined) {
                    _this.player._resetAnimation(simAnimation);
                }
            });
        };
        return AnimationPlayerTask;
    }(EP.Task));
    /**
     * Enum of play modes
    ...
    /**
     * Animation player manager used to play product animations
     *
     * @private
     ...
     * @memberof STU
     */
    var AnimationPlayer = /** @class */ (function (_super) {
         ...
         * @return {number}
         */
        AnimationPlayer.prototype._absoluteModulus = function (n, m) {
            return ((n % m) + m) % m;
        };
        /**
         * @private
         */
        AnimationPlayer.prototype.onInitialize = function () {
            // Product animations are located on the parent actor,
            // we copy them into the behavior
            this._task = new AnimationPlayerTask(this);
            EP.TaskPlayer.addTask(this._task);
        };
        /**
         * @private
         * @param {EP.PlayerContext} iPlayerCtx
         */
        AnimationPlayer.prototype.onExecute = function (iPlayerCtx) {
            ...
            for (var idx = 0; idx < this._registeredAnimations.length; idx++) {
                ...
                var playDirection = anim.simAnim.speedFactor;
                if (anim.simAnim.reverse) {
                    playDirection = -playDirection;
                }
                if (anim.simAnim.bounce) {
                ...
                var isOverLimits = anim.currentTime > anim.simAnim.duration || anim.currentTime < 0;
                if (isOverLimits) {
                    // Handle loop option 
                    if (anim.simAnim.loop) {
                        if (!anim.simAnim.bounce || anim.bounceDirection === -1.0) {
                            if (anim.hasBeenLooping) {
                                var animLoopFinishEvent = new AnimationLoopFinishedEvent(anim.simAnim);
                                anim.simAnim.dispatchEvent(animLoopFinishEvent);
                            }
                            else {
                                anim.hasBeenLooping = true;
                            }
                            if (anim.state === PlayModeEnum.PLAYING) {
                                var animLoopStartedEvent = new AnimationLoopStartedEvent(anim.simAnim);
                                anim.simAnim.dispatchEvent(animLoopStartedEvent);
                            }
                        }
                    }
                    else if (!anim.simAnim.bounce || anim.bounceDirection === -1.0) {
                        // Animation has finished
                        var animHasFinished = new AnimationFinishedEvent(anim.simAnim);
                        anim.simAnim.dispatchEvent(animHasFinished);
                        if (anim.hasBeenLooping) {
                            var animLoopFinishEvent = new AnimationLoopFinishedEvent(anim.simAnim);
                            anim.simAnim.dispatchEvent(animLoopFinishEvent);
                            anim.hasBeenLooping = false;
                        }
                        anim.simAnim.dispatchEvent(new STU.ServiceStoppedEvent("play", anim.simAnim));
                        ...
                        if (anim.currentTime < 0) {
                            anim.currentTime = 0;
                            anim.simAnim._wrapper.jumpToTime(0);
                        }
                        else {
                            anim.currentTime = anim.simAnim.duration;
                            anim.simAnim._wrapper.jumpToTime(anim.simAnim.duration);
                        }
                        // No need to go further if the animation has ended 
                        continue;
                    }
                    // Handle bounce option
                    if (!anim.simAnim.bounce) { // not bouncing 
                        // Handle regular loop
                        anim.currentTime = this._absoluteModulus(anim.currentTime, anim.simAnim.duration);
                    }
                    else { // bouncing
                        ...
                        if (anim.currentTime > anim.simAnim.duration) {
                            // const delta = anim.currentTime - anim.simAnim.duration;
                            // anim.currentTime = anim.simAnim.duration - delta;
                            // Quicker way of doing the two lines above 
                            anim.currentTime = 2.0 * anim.simAnim.duration - anim.currentTime;
                        }
                ...
                if (anim.currentTime < 0 || anim.currentTime > anim.simAnim.duration) {
                    console.error("iTime must between 0 and ".concat(anim.simAnim.duration, "s"));
                    return;
                }
                // Apply computed time to animation
                anim.simAnim._wrapper.jumpToTime(anim.currentTime);
            }
        };
        AnimationPlayer.prototype.onDispose = function () {
         ...
         *
         * @private
         * @param {STU.SimulationAnimation} iAnimation
         */
        AnimationPlayer.prototype.play = function (iAnimation) {
            if (iAnimation === undefined || iAnimation === null) {
                return;
            }
            if (typeof iAnimation.duration !== "number" || iAnimation.duration <= 0) {
                console.error("Simulation Animation duration is invalid");
            }
            ...
            for (var _i = 0, _a = this._registeredAnimations; _i < _a.length; _i++) {
                ...
                if (playingAnim.simAnim === iAnimation) {
            ...
            //because we want to activate the datadisplay
            if (iAnimation._wrapper !== null && iAnimation._wrapper !== undefined) {
                iAnimation._wrapper.initEngineWithAnim(expPointer);
            }
            if (existingState === null) {
                ...
                if (existingState.state === PlayModeEnum.PAUSED) {
                    ...
                    // anim is stopped
                    if (existingState.simAnim.reverse) {
                        if (existingState.currentTime === 0) {
                            existingState.currentTime = existingState.simAnim.duration;
                        }
                    }
                    else if (existingState.currentTime === existingState.simAnim.duration) {
                        existingState.currentTime = 0;
                    }
                    this._resetAnimation(existingState.simAnim);
                }
                existingState.state = PlayModeEnum.PLAYING;
            }
         ...
         * @param {STU.SimulationAnimation} iAnimation
         */
        AnimationPlayer.prototype.pause = function (iAnimation) {
            if (iAnimation === undefined || iAnimation === null) {
                return;
            }
            for (var _i = 0, _a = this._registeredAnimations; _i < _a.length; _i++) {
                var anim = _a[_i];
                if (anim.simAnim === iAnimation) {
                    if (anim.state === PlayModeEnum.PLAYING) {
                        anim.state = PlayModeEnum.PAUSED;
                        var animEvent = new AnimationPausedEvent(iAnimation);
                        anim.simAnim.dispatchEvent(animEvent);
                    }
                    else {
         ...
         * @param {STU.SimulationAnimation} iAnimation
         */
        AnimationPlayer.prototype.stop = function (iAnimation) {
            if (iAnimation === undefined || iAnimation === null) {
                return;
            }
            ...
            this._resetAnimation(iAnimation);
            console.warn("Given animation isn't playing");
        };
        /**
         ...
         */
        AnimationPlayer.prototype._resetAnimation = function (iAnimation) {
            if (iAnimation._wrapper === null || iAnimation._wrapper === undefined) {
                return;
            }
         ...
         * @return {boolean}
         */
        AnimationPlayer.prototype.isPlaying = function (iAnimation) {
         ...
         * @return {boolean}
         */
        AnimationPlayer.prototype.isPlayingBackward = function (iAnimation) {
            for (var _i = 0, _a = this._registeredAnimations; _i < _a.length; _i++) {
                var anim = _a[_i];
                if (anim.simAnim === iAnimation) {
                    if (anim.state === PlayModeEnum.PLAYING) {
                        return anim.bounceDirection < 0 ? !anim.simAnim.reverse : anim.simAnim.reverse;
                    }
                }
            }
            return false;
         ...
         * @return {boolean}
         */
        AnimationPlayer.prototype.isPaused = function (iAnimation) {
         ...
         * @return {boolean}
         */
        AnimationPlayer.prototype.isStopped = function (iAnimation) {
            for (var _i = 0, _a = this._registeredAnimations; _i < _a.length; _i++) {
                var playingAnim = _a[_i];
                if (playingAnim.simAnim === iAnimation) {
         ...
         */
        AnimationPlayer.prototype.setTime = function (iAnimation, iTime) {
            ...
            // Fix for IR-744228 (like IR-744228)
            if (animState.state === PlayModeEnum.PAUSED || animState.state === PlayModeEnum.STOPPED) {
                if (iAnimation._wrapper === null || iAnimation._wrapper === undefined) {
                    ...
                    wrapper.initEngineWithAnim(expPointer);
                }
         ...
         * @return {number}
         */
        AnimationPlayer.prototype.getTime = function (iAnimation) {
            for (var i = 0; i < this._registeredAnimations.length; i++) {
                var anim = this._registeredAnimations[i];
                if (anim.simAnim === iAnimation) {
    ...
    STU.registerManager(AnimationPlayer);
    // Expose in STU namespace.
    STU["AnimationPlayer"] = AnimationPlayer;
    return AnimationPlayer;
});
