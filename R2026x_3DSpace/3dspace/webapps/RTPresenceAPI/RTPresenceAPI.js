...
var RTPresenceAPI = Class.extend(Events,{
    ...
    this.endDate = null;
		...
		PlatformAPI.subscribe(this.login, function(data){
			switch(data.action){
				...
				break;
				case 'setStatus':
					if (data.login === that.login ) that.setStatus(data.status, data.color, data.additionalLabels, data.startDate, data.endDate);
				break;
				case 'logout':
	...
	setStatus : function(status, color, additionalLabels = [], startDate = null, endDate = null) {
		...
		if (this.statusIcon) {
			...
			if (this.displayFonticon) this.statusIcon.style.color = color;
				...
				switch (status) {
          case "Online":
            this.statusIcon.className = class_base + " fonticon-status-ok";
            break;
          case "Busy":
            this.statusIcon.className = class_base + " fonticon-record";
            break;
          case "DND":
            this.statusIcon.className = class_base + " fonticon-status-noway";
            break;
          case "Away":
            this.statusIcon.className = class_base + " fonticon-status-clock";
            break;
          case "Offline":
            this.statusIcon.className = class_base + " fonticon-status-empty";
            break;
          case "OOO":
            this.statusIcon.className =
              class_base + " fonticon-status-out-of-office";
            break;
        }
			}
