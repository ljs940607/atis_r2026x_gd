define('DS/ENXSBGridConnector/6WTag', ['DS/WAFData/WAFData', 'DS/TagNavigatorProxy/TagNavigatorProxy', 'emxUICore', 'DS/ENOjquery/ENOjquery'],
  function(WAFData, TagNavigatorProxy, emxUICore, jQuery) {
      ...
      init: function(StandardFunctions) {
        ...
        if (topAccessWin) {
          ...
          if (!oThis.tnID) {
            ...
            topAccessWin.jQuery(topAccessWin.document).on('dg_data_changed.bps_dg', oThis.handleDrawTags);
            topAccessWin.jQuery(topAccessWin.document).on('dg_selection_changed.bps_dg', oThis.handleTagCollect);
            ...
            destroySBListeners = function() {
              ...
              topAccessWin.jQuery(topAccessWin.document).off(".bps_dg");
            ...
            if (oThis.getTNWindow().isMobile) {
              jQuery(window).on('unload', destroySBListeners);
              jQuery(window).on('unload', removeTagger);
            } else {
              jQuery(window).on('beforeunload', removeTagger);
              jQuery(window).on('unload', destroySBListeners);
            }
