...
define('DS/StuPhysic/StuPhysicRigidBody', [
	...
	function (STU, DSMath, StuCDSDynFactory, CubeActor, PlaneActor, CapsuleActor, CylinderActor, TorusActor, SphereActor, ConeActor, Actor3D) {
		...
		RigidBody.prototype.onInitialize = function (oExceptions) {
			...
			console.debug("RigidBody onInitialize: " + this.getActor().getName());
			// ----------------------------------------------------
			...
			// au activates suivants, on réactive un behavior qui a déjà été initialisé
				...
				console.debug("RigidBody Already intialized: " + this.getActor().getName());
				...
				if (this.RigidBodyActivated === false) {
					...
					this.exclude(); // activate should include this object in the simulation
				}
			}
		};
		...
		RigidBody.prototype.onActivate = function (oExceptions) {
			console.debug("RigidBody onActivate : " + this.getActor().getName());
		...
		RigidBody.prototype.buildCDSDynRigidBody = function () {
			...
			if (PhysManager._TestPolyhedralClashEngine) {
				console.debug("TestPolyhedralClashEngine!!");
		...
		RigidBody.prototype.onDeactivate = function () {
			...
			console.debug("RigidBody onDeactivate: " + this.getActor().getName());
		...
		RigidBody.prototype.updateRigidPos = function () {
			...
			console.debug("CDSDynRigidBody INCLUDE: " + this.getActor().getName());
		 ...
		 * @private
		 */
		RigidBody.prototype.exclude = function () {
			// keep the valid CDSDynRigidBody, inertia computation, etc but
			// store object outside of physical world boundaries so it 
			// does not contribute to simulation
			console.debug("CDSDynRigidBody exclude: " + this.getActor().getName());
			if (this.CDSDynRigidBody !== null && this.CDSDynRigidBody !== undefined) {
				var PhysManager = STU.PhysicManager.getInstance();
