...
define('DS/StuMiscContent/StuPlaneController', [
	...
	function (STU, EP, Behavior, EventServices, Event, DSMath, Actor3D, ProductActor, SubProductActor, PathActor) {
		...
		PlaneController.prototype.onActivate = function (oExceptions) {
			...
			this._sceneTransfo = this._plane.getSceneTransform();
		...
		PlaneController.prototype.isZRaycastCollide = function (vector) {
			...
			var intersectArray = renderManager._pickFromRay(ray, true, false, this._sceneTransfo); 
		...
		PlaneController.prototype.fliesAlong = function (iPath) {
			...
			if (this._isLanded) {
				...
				if (iPath.getValue(0, this._sceneTransfo).z > this.actor.getPosition().z) {
					this._flightAltitude = iPath.getValue(0, this._sceneTransfo).z;
				}
				else {
					this._flightAltitude = this._altitudeOffset + (this.flightAltitude * 0.5);
				}
			...
			/*else if(distanceToTheBeginningOfThePath > 1000){
				...
				var startPoint = this._flyingPathTarget.getValue(0, this._sceneTransfo);
				var lastPoint = this._flyingPathTarget.getValue(0.01, this._sceneTransfo);
		...
		PlaneController.prototype.drivesOn = function (iPath) {
			...
			var startPoint = this._drivingRunway.getValue(0, this._sceneTransfo);
			var lastPoint = this._drivingRunway.getValue(0.01, this._sceneTransfo);
			var direction = DSMath.Vector3D.sub(lastPoint, startPoint);
		...
		PlaneController.prototype.takesOff = function (iPath) {
			...
			if (startsStraightForward) {
				...
				startPoint = this._takeOffRunway.getValue(0, this._sceneTransfo).clone();
				lastPoint = this._takeOffRunway.getValue(0.01, this._sceneTransfo);
		...
		PlaneController.prototype.landsOn = function (iPath) {
			...
			if (this._isLanded) {
				...
				var startPoint = this._landingRunway.getValue(0, this._sceneTransfo);
				var lastPoint = this._landingRunway.getValue(0.01, this._sceneTransfo);
		...
		PlaneController.prototype.updateFliesAlong = function (iDelta) {
			...
			if (this._flyingAlongState === -3) {
				...
				var futurPosition = this._plane.getPosition().clone().add(futurePositionOffset);
				if (futurPosition.z > this._flyingPathTarget.getValue(0, this._sceneTransfo).z + heightTolerance) {
					this.moveDownFunc(iDelta, 0.25);
				}
				else if (futurPosition.z < this._flyingPathTarget.getValue(0, this._sceneTransfo).z - heightTolerance) {
			...
			//
				...
				if (this._actionTotalTime < this._actionCurrentTime) {
					this._flyingAlongState = 3;
					this._localDestination = this._flyingPathTarget.getValue(0, this._sceneTransfo);
					this._actionCurrentTime = this._actionTotalTime;
				...
				var dist = this.DistanceToPosition(this._flyingPathTarget.getValue(0, this._sceneTransfo));
				...
				if (this._actionCurrentTime > this._actionTotalTime) {
					...
					if (dist < 50) {//if(dist < 1000){
						...
						t = t > 1 ? 1 : t;
						var newPosition = DSMath.Vector3D.lerp(this._positionTst, this._flyingPathTarget.getValue(0, this._sceneTransfo), t);
			...
			//step 2: follow the path
				...
				var nextPosition = this._flyingPathTarget.getValue(this._currentDistanceFollowPath / this._pathLength, this._sceneTransfo);
				...
				if (followPath) {
					...
					nextPositionVarTst = this._flyingPathTarget.getValue(offset / this._pathLength, this._sceneTransfo);
				}
				...
				var nextTargetedPosition = this._flyingPathTarget.getValue(nextTargetedDistance / this._pathLength, this._sceneTransfo);
		...
		PlaneController.prototype.updateDrivesOn = function (iDelta) {
			...
			if (this._drivingOnState === 0) {
				...
				var nextPosition = this._drivingRunway.getValue(this._currentDistanceFollowPath / this._pathLength, this._sceneTransfo);
				...
				if (followPath) {
					...
					nextPositionVarTst = this._drivingRunway.getValue(offset / this._pathLength, this._sceneTransfo);
				...
				var nextTargetedPosition = this._drivingRunway.getValue(nextTargetedDistance / this._pathLength, this._sceneTransfo);
				...
				if (nextTargetedDistance < this._pathLength) {
					...
					var tst = this._drivingRunway.getValue(this._currentDistanceFollowPath / this._pathLength, this._sceneTransfo);
					var tst2 = this._drivingRunway.getValue((this._currentDistanceFollowPath + u) / this._pathLength, this._sceneTransfo);
		...
		PlaneController.prototype.updateTakeOff = function (iDelta) {
			...
			if (this._takeOffState === 0) {
				...
				var nextPosition = this._takeOffRunway.getValue(this._currentDistanceFollowPath / this._pathLength, this._sceneTransfo);
				nextPosition = DSMath.Vector3D.sub(nextPosition, this._worldLowPositionOffset);
				...
				var nextTargetedPosition = this._takeOffRunway.getValue(nextTargetedDistance / this._pathLength, this._sceneTransfo);
				nextTargetedPosition = DSMath.Vector3D.sub(nextTargetedPosition, this._worldLowPositionOffset);
				...
				if (nextTargetedDistance < this._pathLength) {
					nextTargetedPosition = this._takeOffRunway.getValue(nextTargetedDistance / this._pathLength, this._sceneTransfo);
					nextTargetedPosition = DSMath.Vector3D.sub(nextTargetedPosition, this._worldLowPositionOffset);
		...
		PlaneController.prototype.updateLandsOn = function (iDelta) {
			...
			if (this._landingOnState === 0) {
				...
				if (this._actionTotalTime < this._actionCurrentTime) {
					...
					this._localDestination = this._landingRunway.getValue(0, this._sceneTransfo);
					this._actionCurrentTime = this._actionTotalTime;
		...
		PlaneController.prototype.analysePathUserScale = function (iPath) {
			...
			for (var i = 0; i <= division; i++) {
				var pos = iPath.getValue(i / division, this._sceneTransfo);
				if (true) {
