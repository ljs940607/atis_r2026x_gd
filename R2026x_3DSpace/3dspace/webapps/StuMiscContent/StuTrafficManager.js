...
define('DS/StuMiscContent/StuTrafficManager', [
	...
	function (STU, EP, Behavior, DSMath, Collection, PathActor) {
		...
		*/
		...
		TrafficManager.prototype.computeTrafficWheelIntersections = function (currentRoad, iTrackingPosition, iRightVector, iGroundIncluded) {
			...
			if (iGroundIncluded) {
				rightIntersection = this.getFirstIntersectPointDown(rightOrigin, currentRoad.getSceneTransform());
				leftIntersection = this.getFirstIntersectPointDown(leftOrigin, currentRoad.getSceneTransform());
			}
			else {
				var upVector = new DSMath.Vector3D(0, 0, 1);
