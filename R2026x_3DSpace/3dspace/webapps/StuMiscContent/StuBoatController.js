define('DS/StuMiscContent/StuBoatController',
	...
	function (STU, EP, Behavior, Task, EventServices, Event, DSMath, Actor3D) {
		...
		BoatController.prototype.onExecute = function (iDelta) {
			...
			//ABA57: IR-1317750 --> if frame taking too long, boat will go too far under the water and then the algorithm of buoyancy
			// will make a big bounce and boat will fly in the sky
			if(iDelta.deltaTime > 1000)
			{
				return;
			}
		...
		BoatController.prototype.updateFollowTrajectory = function (iPath, iDeltaTime) {
			...
			if (iPath !== undefined || iPath !== null) {
				...
				traveledDistanceInOneSec = ((1 - this._stickCoefficient) * traveledDistanceInOneSec) + ((this._stickCoefficient) * 1e-6);
		...
		BoatController.prototype.computeTrajectory = function () {
			...
			var divisionMin = div;
			var divisionMax = Math.max(div, Math.min(div * 10, this._pathLength / (this._length / 2)));  // clamp by ratio of length of the boat compared to the path itself
