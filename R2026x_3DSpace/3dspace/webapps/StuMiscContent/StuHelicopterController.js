...
define('DS/StuMiscContent/StuHelicopterController', [
	...
	function (STU, EP, Behavior, EventServices, Event, DSMath, Actor3D, PointActor, Location) {
		...
		var HelicopterController = function () {
			 ...
			 */
			this._sceneTransfo = new DSMath.Transformation();
		...
		HelicopterController.prototype.onActivate = function (oExceptions) {
			...
			this._sceneTransfo = this._helicopter.getSceneTransform();
		...
		HelicopterController.prototype.computeNextPathDestination = function (Path) {
			var second = 0.0;
			var newDestination = Path.getValue(this._currentFollowPathDist / this._pathLength, this._sceneTransfo);
			do {
				second++;
				...
				newDestination = Path.getValue(this._currentFollowPathDist / this._pathLength, this._sceneTransfo);
			}
		...
		HelicopterController.prototype.fliesAlong = function (actor) {
			...
			this._destination = this._target.getValue(0, this._sceneTransfo);
		...
		HelicopterController.prototype.isCloseToLand = function (iVector) {
			...
			posWorld.applyTransformation(this._sceneTransfo);
			rayDirection.applyTransformation(this._sceneTransfo);
		...
		HelicopterController.prototype.isRayCastCollide = function (origin, direction, length) {
			...
			originWorld.applyTransformation(this._sceneTransfo);
			rayDirection.applyTransformation(this._sceneTransfo);
		...
		HelicopterController.prototype.closestDstRayCollision = function (origin, direction, length) {
			...
			if (intersectArray.length > 0) {
				...
				var invTransfoScene = this._sceneTransfo.getInverse();
