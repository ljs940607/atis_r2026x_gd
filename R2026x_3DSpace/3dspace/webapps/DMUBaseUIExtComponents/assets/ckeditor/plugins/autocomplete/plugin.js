		...
		setPosition: function( rect ) {
			...
			function getEditorViewportRect( editor ) {
				...
				if ( CKEDITOR.env.iOS && !editable.isInline() ) {
					...
					// return editable.isInline() ? editable.getClientRect( true ) : editor.window.getFrame().getClientRect( true ); commented to avoid IR-1205967-3DEXPERIENCER2024x error
					return editor.window.getFrame() ? editor.window.getFrame().getClientRect( true ) : editable.getClientRect( true );
				}
			}
		},
