...
if "%1"=="" (
    if "%MkmkJREDIR%"=="" (
    	...
    	"%MkmkJREDIR%"\bin\java.exe -cp steal\rhino\js.jar org.mozilla.javascript.tools.shell.Main
    )
	GOTO END
)
...
if "%1"=="-d" (
    if "%MkmkJREDIR%"=="" (
    	...
    	"%MkmkJREDIR%"\bin\java.exe -classpath funcunit/java/selenium-java-client-driver.jar;steal/rhino/js.jar org.mozilla.javascript.tools.debugger.Main
    )
	GOTO END
)
...
set ARGS=%ARGS:\=/%
if "%MkmkJREDIR%"=="" (
    java -Xmx8192m -Xss4096k -cp %CP% org.mozilla.javascript.tools.shell.Main -opt -1 -e _args=%ARGS% -e load('%FILENAME%')
) ELSE (
    "%MkmkJREDIR%"\bin\java.exe -Xmx8192m -Xss4096k -cp %CP% org.mozilla.javascript.tools.shell.Main -opt -1 -e _args=%ARGS% -e load('%FILENAME%')
)
