...
if "%1"=="" (
    if "%MkmkJREDIR%"="" (
    	...
    	"%MkmkJREDIR%"\bin\java -cp steal\rhino\js.jar org.mozilla.javascript.tools.shell.Main
    )
	GOTO END
)
...
if "%1"=="-d" (
    if "%MkmkJREDIR%"="" (
        ...
        "%MkmkJREDIR%"\bin\java -classpath funcunit/java/selenium-java-client-driver.jar;steal/rhino/js.jar org.mozilla.javascript.tools.debugger.Main
    )
	GOTO END
)
...
echo "NWT DEBUG: MkmkJREDIR = %MkmkJREDIR%"
if "%MkmkJREDIR%"="" (
    java -Xmx512m -Xss1024k -cp %CP% org.mozilla.javascript.tools.shell.Main -opt -1 -e _args=%ARGS% -e load('%FILENAME%')
) ELSE (
    "%MkmkJREDIR%"\bin\java -cp %CP% org.mozilla.javascript.tools.shell.Main -opt -1 -e _args=%ARGS% -e load('%FILENAME%')
)
