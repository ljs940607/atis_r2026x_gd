...
// check that we have a document,
	...
	request: function( options, success, error ) {
	  ...
	  //if(window.console && console.log) console.log("steal.js request(168) pulling "+options.src);
...
ConfigManager.defaults.types = {
	...
	"text": function( options, success, error ) {
	  //NWT 4/13/2015 - use the window.steal.request object, as this might be overridden to compensate for proxy servers
	  window.steal.request(options, function( text ) {
			options.text = text;
			success(text);
		}, error);
