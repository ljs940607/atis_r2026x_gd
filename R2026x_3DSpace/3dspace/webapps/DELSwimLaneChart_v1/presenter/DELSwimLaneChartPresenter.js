/// <amd-module name="DS/DELSwimLaneChart_v1/presenter/DELSwimLaneChartPresenter"/>
define("DS/DELSwimLaneChart_v1/presenter/DELSwimLaneChartPresenter", ["require", "exports", "DS/DELSwimLaneChart_v1/utils/StaticAttributes", "DS/DELSwimLaneChart_v1/utils/LayoutUtils", "DS/DELSwimLaneChart_v1/services/presenterServices/LayoutManager", "DS/DELSwimLaneChart_v1/model/SwimLaneChartColumn"], function (require, exports, StaticAttributes_1, LayoutUtils_1, LayoutManager_1, SwimLaneChartColumn_1) {
    "use strict";
    ...
    class SwimLaneChartPresenter {
        ...
        updateVisibleElts() {
            ...
            let inVisibleNodes = this._visibleNodes.filter((n) => !newVisibleNodesIds.includes(n.id));
            // groups
            ...
            if (this._transVector[2] === 1 && this._fitToWindowScale === 1) {
                ...
                const newStickyNodesIds = this._stickyNodes.map((n) => n.id);
                inVisibleNodes = inVisibleNodes.filter((n) => !newStickyNodesIds.includes(n.id));
        ...
        handleResizeObserver() {
            if (this._fitToWindowScale !== 1 || this._transVector[2] !== 1)
                return;
            const gridContainer = this._swimLaneChartView.getGridContainer();
        ...
        */
        ...
        handlePointerDown(e) {
            ...
            if (!["node", "childElement", "nodeHeaderGroup", "foreignNode", "nodeHeaderResponsiveText", "nodeHeader", "nodeHeaderText", "bodyTextContent"].includes(type))
                return;
            if (!targetElt)
                return;
            e.preventDefault();
            this._swimLaneChartModel.selectSwimLaneChartNode(nodeId);
        ...
        handleDblClick(e) {
            ...
            if (!["node", "childElement", "nodeHeaderGroup", "foreignNode", "nodeHeaderResponsiveText", "nodeHeader", "nodeHeaderText", "bodyTextContent"].includes(parenttype))
                return;
            const targetElt = this._swimLaneChartModel.getSwimLaneChartNodeById(nodeId);
            if (!targetElt)
                return;
            ...
            if (type === "nodeHeaderResponsiveText" || type === "nodeHeader" || type === "nodeHeaderText" || type === "headerLeftIcon" || type === "headerRightIcon") {
                if (targetElt.isMinimized)
                    this.expand(nodeId);
                else
                    this.minimize(nodeId);
            }
            else if (this._props.onNodeDblSelect && targetElt)
         ...
         * @returns
         */
        getOnContextMenuTarget(target) {
            var _a, _b, _c;
            const [type, nodeId] = (_c = (_b = (_a = target.parentElement) === null || _a === void 0 ? void 0 : _a.id) === null || _b === void 0 ? void 0 : _b.split("__")) !== null && _c !== void 0 ? _c : [];
            if (["columnSeparatorGroup", "columnHeader", "columnHeaderText"].includes(type))
                return null;
            const targetElt = this._swimLaneChartModel.getSwimLaneChartNodeById(nodeId);
            if (!targetElt)
                return null;
        ...
        updateNode(props) {
            ...
            if (node.parentId !== oldParentId) {
                ...
                if (oldParent)
                    this._layoutManager.positionChildElementsInsideParent(oldParent);
                const newParent = this._swimLaneChartModel.nodes.get(node.parentId);
                if (newParent) {
                    newParent.node.type = "CompositeNode";
                    this._layoutManager.positionChildElementsInsideParent(newParent);
                }
                if (!oldParentId)
        ...
        /**
         * Method to expand all swimlane nodes
         */
        expandAll() {
            this._swimLaneChartModel.getSwimLaneChartNodes().forEach((n) => this._swimLaneChartModel.expandSwimLaneChartNode(n.id));
            if (this._transVector[2] === 1 && this._fitToWindowScale === 1) {
                this.resetStickyTransformation();
                this._swimLaneChartView.resetScrollbar();
            }
        ...
        */
        ...
        async exportToSvg() {
            ...
            parentElt.setAttribute("height", `${this._layoutManager.getSwimLaneChartHeight()}`);
            parentElt.setAttribute("viewBox", `${0} ${-2 * StaticAttributes_1.gridStep} ${this._layoutManager.getSwimLaneChartWidth()} ${this._layoutManager.getSwimLaneChartHeight()}`);
        ...
        renderInvisibleElts() {
            // nodes
            const currentVisibleNodesIds = this._visibleNodes.map((n) => n.id);
            const inVisibleNodes = this._swimLaneChartModel.getSwimLaneChartVisibleColumnNodes().filter((n) => !currentVisibleNodesIds.includes(n.id));
            // groups
            const currentVisibleGroupsIds = this._visibleGroups.map((gr) => gr.id);
