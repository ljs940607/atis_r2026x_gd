...
var bpsWidgetPreferences = {
	...
	template: {
		...
		gear: function (obj) { 
			var othis = this;
			return jQuery('<div class="settings-button preferences" title="' + bpsWidgetConstants.str.Preferences + '"></div>').on('click',function(e) {
				e.stopPropagation();
				...
				var offset = jQuery(this).offset() || { left: 0, top: 0 };
				var menuWidth = prefPane.children('.preference-menu').width() || 0;
				var thisHeight = jQuery(this).height() || 0;

				prefPane.css({
					'left': (offset.left - menuWidth) + 'px',
					'top': thisHeight + 'px'
				});
...
//			var sortOptions = jQuery('<div class="preference-attr-body"></div>');
		...
		buildPreferences:function(obj, userViews, sortFields){
			...
			jQuery(".apply", menu).on('click',function (e) {
				var view;
				var sortField;
				var sortDirection;
			...
			jQuery(".cancel", menu).on('click',function (e) {
				menu.parent().toggle();
			});
