...
//					othis.cropTreeItems();
	...
	this.$ds_timeline.find('.scroll-today').on('click',
		function (event) {
			othis.today();
		}
	);

	this.$ds_timeline.find('.zoom-viewall').on('click',function (event) {
	...
	this.$ds_timeline.find('.timeline-tools-version').on('click', function (event) {
	...
	this.$ds_timeline.find('.timeline-tools-save').on('click', function (event) {
		var node = othis.__getActiveNode();
		if (Timeline.isDeliverable(node)) {
			othis.$ds_timeline.trigger({
				type: "createTemplate",
	...
	this.$ds_timeline.find('.set-dates').on('click', function (event) {
		othis.$ds_timeline.trigger({
			type: "setDates",
	...
	this.$ds_timeline.find('.timeline-tools-activate').on('click', function(event) {
		var node = othis.__getActiveNode();
		if (Timeline.isDeliverable(node)) {
			othis.$ds_timeline.trigger({
				type: "activatePlan",
	...
	this.$ds_timeline.find('.timeline-tools-expand').on('click', function (event) {
		var selected = othis.__getActiveNode();
		othis.$ds_timeline.trigger({
			type: "expandNode",
			node: selected
	...
	this.$tools_lock.on('click', function (event) {
	...
	this.$tools_status.on('click', function (event) {
	...
	this.$tools_small_font.on('click', function (event) {
		othis.decreaseFont();
		othis.$timeline.find('.cart').each(function(){Timeline.cascadeGates(jQuery(this))});
	});

	this.$tools_large_font.on('click', function (event) {
		othis.increaseFont();
		othis.$timeline.find('.cart').each(function(){Timeline.cascadeGates(jQuery(this))});
	});

	this.$tools_search.on('click', function (event) {
	...
	this.$tools_people.on('click', function (event) {
		othis.$ds_timeline.trigger({
			type: "assignAccess",
	...
	this.$tools_remove_item.on('click', function (event) {
	...
	// touch and scroll
	this.$dateline_container.on('mousedown',function (event) {
	...
	this.$dateline_container.on('mouseup',function (event) {
		jQuery.data(this, 'down', false);
		jQuery.data(this, 'timeMachine', false);
	});
	this.$dateline_container.on('mousemove',function (event) {
	...
	this.$dateline_container.on('mousewheel DOMMouseScroll wheel', function(event) {
        event.preventDefault();
        var dir = 0;
	...
	this.$timeline_container.on('mousedown',function (event) {
	...
	this.$timeline_container.on('mouseup',function (event) {
		jQuery.data(this, 'down', false);
		var $target = jQuery(event.target);
		if ($target.hasClass("cart") || $target.hasClass("td") || $target.hasClass("selection-box")) {
			//TODO should probably change cursor with a class change
	...
	this.$timeline_container.on('mousemove',function (event) {
	...
	this.$timeline_container.on('scroll',(function (event) {
		othis.setPreference(Timeline.PREFERENCE_SCROLL_TOP, JSON.stringify(jQuery(this).scrollTop()));
	});
	
	this.$timeline_container.on('mousewheel', function(event, delta) {
	...
	this.$ds_timeline.on("setDates", function (event) {
		...
		$popup.find('input:text:visible:first').trigger('blur');
	...
	this.$ds_timeline.on("createGatePopup", function (event) {
		...
		$popup.find('#select-list select').on('change',(function() {
