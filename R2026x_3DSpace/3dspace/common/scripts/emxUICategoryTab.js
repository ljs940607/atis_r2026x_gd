...
var emxUICategoryTab = {
		...
		init : function(catObj){
			...
			if(catObj.objectId && catObj.objectId != "null"){
					...
					if(getTopWindow().isfromIFWE){
						getTopWindow().toppos = jQuery('#ExtpageHeadDiv').height() || 0;
						divph.style.top = "0px";
					} else {
						if(emxUIConstants.TOPFRAME_ENABLED){
							getTopWindow().toppos = (jQuery('#topbar').height() || 0) + (jQuery('#navBar').height() || 0) + (jQuery('#ExtpageHeadDiv').height() || 0);
							divph.style.top = jQuery('#topbar').height() && jQuery('#navBar').height() ? jQuery('#topbar').height() + jQuery('#navBar').height() + "px" : "0px";
						}else{
							getTopWindow().toppos = (jQuery('#pageHeadDiv').height() || 0) + (jQuery('#ExtpageHeadDiv').height() || 0);
			...
			if(hasCatMenu) {
				...
				if(getTopWindow().objStructureFancyTree && getTopWindow().objStructureFancyTree.fromActivateNode){
					...
					var pageContentDivLeft = (jQuery("div#leftPanelMenu").width() || 0) + (jQuery("div#panelToggle").width() || 0);
					jQuery("div#pageContentDiv").css('left', (jQuery("div#leftPanelMenu").width() || 0) + (jQuery("div#panelToggle").width() || 0) + 'px');
			...
			jQuery(contentFrame).on('unload', function(){
				getTopWindow().emxUICategoryTab.destroy();
			});

				if(catObj.objectId && catObj.objectId != 'null'){
					
					jQuery(contentFrame).on('resize', function(){
		...
		redrawPanel : function(){
			...
			var widthTabBar = jQuery("div#ExtpageHeadDiv").width() || 0;	
			if(jQuery('#appsPanel').is(":visible")){				
				widthTabBar -= jQuery('#appsPanel').innerWidth() || 0;
			}
			if (jQuery('#rightSlideIn').is(":visible")) {
			  widthTabBar -= jQuery("div#rightSlideIn").innerWidth() || 0;
			}
			
			if (jQuery('#leftSlideIn').is(":visible")) {
			  widthTabBar -= jQuery("div#leftSlideIn").innerWidth() || 0;
			}
			
			var widthImage = 100;
			var widthName = jQuery("div#divExtendedHeaderName").innerWidth() || 0;
			var widthDescription = jQuery("div#divExtendedHeaderDescription").innerWidth() || 0;
			var widthDetails = jQuery("div#divExtendedHeaderDetails").innerWidth() || 0;
			var widthDocuments = jQuery("div#divExtendedHeaderDocuments").innerWidth() || 0;
			var widthNavButtons = jQuery("div#divExtendedHeaderNavigation").innerWidth() || 0;
		...
		redraw : function(){
			...
			var liTab;
			var totalWidth = (selectedTab.outerWidth() || 0) + (parseInt(selectedTab.css('margin-right')) || 0);
			...
			for(var inc=0; inc < tabs.length; inc++){
				...
				if(liTab.get(0) != selectedTab.get(0)){
					totalWidth += (liTab.outerWidth() || 0) + (parseInt(liTab.css('margin-right')) || 0);
				}
				if ((tab.width() || 0) > (totalWidth + (overflowButton.outerWidth() || 0))){
					liTab.show();
		...
		closeOverflowEvent : function (){
			...
			emxUICore.iterateFrames(function (objFrame) {
				if(objFrame){
					jQuery(objFrame.document).on("mousedown",function(e){
		...
		menu : {
			...
			addOverflow : function(){
				...
				overflow.bind("click",function(){
					...
					var overflowTop = (jQuery('div#globalToolbar').height() || 0) + (jQuery('div#navBar').height() || 0) + (jQuery('div#tabBar').height() || 0);
					categoryDiv.css('top', overflowTop +'px');
					var left = (jQuery(this).offset().left || 0) - (categoryDiv.outerWidth() || 0) + (overflow.outerWidth() || 0) + "px";
					categoryDiv.css('left', left);
					categoryDiv.show();
					//to adjust the height of the overflow div
					var reqheight = jQuery(window).height() - overflowTop - 10;
					if (categoryDiv.height() >= (reqheight || 0)) {
  						jQuery('div.menu-content', categoryDiv).height(reqheight);
					} else {
  						jQuery('div.menu-content', categoryDiv).height(jQuery('ul', categoryDiv).height() || 0);
					}
			...
			createDOM : function(menuObj){
				...
				var objli, objMenuLi, objThis;
				var  catObj = this;
				menuObj.totalWidth = overflowButton.outerWidth() || 0;
				for (var cmd=0; cmd < menuObj.items.length; cmd++) {
					objThis = menuObj.items[cmd];
					if(objThis.grayout == "true"){
						continue;
					...
					menuObj.totalWidth += (objli.outerWidth() || 0) + (parseInt(objli.css('margin-right')) || 0);
					...
					if((tab.width() || 0) < (menuObj.totalWidth)){
						objli.hide();
