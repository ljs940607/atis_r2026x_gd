...
emxUITabbedPortalContainer.prototype.buildDOM = function _emxUITabbedPortalContainer_buildDOM() {
        ...
        for (var i=0; i < this.channels.length; i++) {
        	if(!overflow){
                ...
                tabsetWidth = jQuery(objTR).width() || 0;
...
emxUIPortalTab.prototype.bringToFront = function _emxUIPortalTab_bringToFront() {
	...
	if (isMaxIE8){
		if(isMaximised === false){
			jQuery('.pv-row').on('resize',function () {
...
function redrawTabsetItems(){							
	...
	for(var l = 0; l < objRow.portal.rows.length; l++){	
		...
		for(var ll = 0; ll < objRow.portal.rows[l].containers.length; ll++){						
			...
			var channelWidth = (jQuery(objRow.portal.rows[l].containers[ll].element).width() || 0) - 60;							
			...
			for ( i=0; i < tabset1.tabs.length; i++) {						
				...
				var channelWidth1 = jQuery(objTR).width() || 0;										
...
//            }
		...
		var pvOffset = jQuery('#divPowerView').offset() || { left: 0, top: 0 };
		var pvBox = {
			top: (jQuery('#divPowerView').offset().top || 0) + "px",
  			left: (jQuery('#divPowerView').offset().left || 0) + "px",
			right:"0px", //TODO
			bottom: "0px" // TODO
		};
		...
		var chanOffset = jQuery(chan).offset() || { left: 0, top: 0 };
		var chanBox = {
			top: chanOffset.top + "px",
			left: chanOffset.left - pvOffset.left + "px",
			right: (jQuery('#divPowerView').width() || 0) + (pvOffset.left || 0) - (chanOffset.left || 0) - (jQuery(chan).width() || 0) + "px",
			bottom: (jQuery('#divPowerView').height() || 0) + (pvOffset.top || 0) - (chanOffset.top || 0) - (jQuery(chan).height() || 0) + "px"
		}
    ...
    return { // PUBLIC
        ...
        init: function(objPortal) {
			...
			jQuery(".pv-row").each(function () {
				var colLength = jQuery(".pv-col", this).length;
				if(colLength > 1){
				if(colLength > 2){
					...
					if(jQuery(".pv-channel", this).length>2){
            ...
            jQuery(".pv-container").each(function () {
				...
				rowData.each(function () {                	
                	if(numRows>1 && jQuery(".pv-col", this).not(".pv-col.closed-cols").length>0){                		
                		jQuery(this).css("height", updateLSValue(jQuery(this).attr("id"), ((jQuery(".pv-container").height() || 0) / (numRows || 1)) + "px"));
                	}
                });
                ...
                if (!_ALWAYS_FIT) { // EXPERIMENTAL, not finished
                    ...
                    if (numRows > 2) {
                        ...
                        jQuery(".pv-row", this).each(function () {
                            ...
                            h = Math.max(h || 0, jQuery(this).height() || 0);
            ...
            jQuery(".pv-row").each(function () {
            	...
            	if(colData.length >1){
            		...
            		jQuery(this).css("width", (jQuery(this).width() || 0)-SPLITTER_WIDTH);
            	});
            	}          	
            	
                if (jQuery(".pv-col", this).length > 1) { // but only if it has more than one channel
                    jQuery(this).splitter({
                        splitVertical: true
                    });
                    _splitters = _splitters.add(this);
            ...
            jQuery(".pv-row").each(function () {
            	...
            	if(colData.length>0){
                    	...
                    	if(noOfChannels>0){
                    		if (noOfChannels > 1) { // but only if it has more than one channel
                    			channelData.each(function() {  
                    				jQuery(this).css("height",updateLSValue(jQuery(this).attr("id"), ((($(this).parents(".pv-col").height()|| 0)-SPLITTER_WIDTH)/noOfChannels)+"px"));
			});
                        		jQuery(this).splitter({
                        			splitHorizontal: true,
                    			...
                    			channelData.each(function() { 
                    				var parentHeight = $(this).parents(".pv-col").height() || 0;
                    				var perc = ((100 * (parentHeight - 4)) / parentHeight) || 0;
                				...
                				if(noOfChannels>1){
                					channels.each(function(){
                						...
                						if(channels1 == noOfChannels){
                							jQuery(this).css("height",((jQuery(column).height() || 0)-prevChannel));
                						}
            		...
            		if (jQuery(".pv-channel", this).length > 1) { // but only if it has more than one channel
                        jQuery(this).splitter({
                            splitVertical: true
                        });
                        _splitters = _splitters.add(this);
