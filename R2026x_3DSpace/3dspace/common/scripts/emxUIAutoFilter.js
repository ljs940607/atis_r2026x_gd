...
var emxUIAutoFilter = {
    ...
    _setupStateListeners: function () { //Event pooling for commonly updated states
        var objThis = this;
        jQuery(this.sb.document).on('UPDATE_FACET_ATTR_STATE', function (e, data) {
	...
	_scrollToLastSelectedFilter: function (){
        if(this._lastSelectionColIndex || this._lastSelectionValueIndex){
			if(this._lastSelectionValueIndex){
	            ...
	            if($(filterValueSelector).length == 1){ 	            	
    ...
    _initializeInnerControls: function () { //initializes all facet actions
        ...
        jQuery('div.facet-attr-head button.arrow').on('click',function (event) {
        ...
        jQuery('button.btn-filter-inactive, button.btn-filter-active').on('click',function (event) {
        ...
        jQuery('button.btn-colorize-inactive, button.btn-colorize-active').on('click',function (event) {
        ...
        jQuery('div.facet-functions button.colorize').on( "mouseenter",function () {
            if (objThis.current_color_btn !== this) { //don't hover on current button
                jQuery(this).toggleClass('hover');
            }
        });
        // set show hide for facet functions
        jQuery('div.facet-attr').on( "mouseenter",function () {
            jQuery(this).toggleClass('hover');
        });
        // check / uncheck boxes
        jQuery('div.facet-body div#multi-select ul li a#multi-select-checkbox').on('click',function (event) {
        ...
        jQuery('div.facet-attr-body span.select-all a.checkbox').on('click',function (event) {
        	jQuery(this).toggleClass('selected');
        ...
        // check / uncheck boxes
        jQuery('div.facet-attr-body ul li a.checkbox').on('click',function (event) {
        ...
        jQuery('div.facet-attr-body ul li a.checkbox + label').on('click',function (event) {
            event.preventDefault();
    ...
    _initializeOuterControls: function () { //initializes controls like reset and close on the slidein
        ...
        // reset the page
        jQuery('a.action-reset').on('click',function (event) {
            event.preventDefault();
