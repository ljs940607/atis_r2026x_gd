                ...
                emxUICore.iterateFrames(function (objFrame) {
            	  ...
            	  try{
            		  ...
            		  if(objRef.className == "search-widget"){
	                 	  ...
	                 	  tempref = tempref[0];
            			  intX = (jQuery(tempref).offset().left || 0) - Math.max(0, ((jQuery(tempref).offset().left || 0) + (jQuery(this.layer).width() || 0) - (jQuery(window).width() || 0)));
            			  k = 8;

            		  } else {
            			  intX = (jQuery(objRef).offset().left || 0) - Math.max(0, ((jQuery(objRef).offset().left || 0) + (jQuery(this.layer).width() || 0) - (jQuery(document).width() || 0)));
            		  }
            		  intY = (jQuery(objRef).offset().top || 0) + objRef.offsetHeight;
            		  intY = intY - k;
            	  }catch(e){
				  ...
				  var toolbar = objRef.parentNode.parentNode.parentNode.parentNode.id;
  				  if(toolbar == "globalToolbar"){
						if(objRef.className != "search-widget"){
		  					  try {
		  						  intX = (jQuery(objRef).offset().left || 0) + (jQuery(objRef).width() || 0) - (jQuery(this.layer).width() || 0);
		  					  } catch(e){
...
emxUICorePopupMenu.prototype.show1 = function _emxUICorePopupMenu_show1(objRef, strDir) {
        ...
        /*if (isMinIE55 && isWin) {
                ...
                if(toolbar == "globalToolbar" && emxUICore.getWinWidth(this.displayWindow) < ((jQuery(this.layer).width() || 0) + intX)){
                	intX = intX - (jQuery(this.layer).width() || 0)+objRef.offsetWidth;
                }
 ...
 * To Caculate the Height of menu and show teh scroll bar if needed.*/
   function adjustMenuHeight(menuElem, isMyDesk, strDir){
	   ...
	   if ((jQuery(menuElem[0].offsetParent).hasClass("expanded"))){
			hasSubMenu= true;
	   }
	   if((jQuery(':visible', menuElem).last().offset() && jQuery(':visible', menuElem).last().offset().top < tempHt) ||  (menuElem[0].clientHeight || 0) < tempHt){
       if( !hasSubMenu){
