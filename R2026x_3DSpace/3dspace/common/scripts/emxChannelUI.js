...
var emxChannelUI={
		...
		template: {
			...
			channel_head_list_iteam_filter_button : function(){
				...
				$(filterButton).on('click',(function(e){
					...
					var eventPos = $(e.target).offset() || { left: 0, top: 0 };
					e.stopPropagation();
					...
					eventPos.left=eventPos.left - (filterElem.width() || 0) +5;
		...
		_buildFilterPopup:function (channel,channelId){
			...
			$(".apply",filterPopupMenu).on('click',(this._applyFilter);
			$(".reset",filterPopupMenu).on('click',(this._resetFilter);
