...
var lwRTE = function (textarea, options) {
	...
	if(document.designMode || document.contentEditable) {
		...
		if(this.ok_cancel){
			...
			if(this.callback){
				var self = this;
				jQuery('#rte_ok').on('click',function(){self.callback()});
				jQuery('#rte_cancel').on('click',function(){jQuery('.formLayer').remove();});
			}
		}
		var resizer = new lwRTE_resizer(textarea);
		
		jQuery(".rte-resizer a", jQuery(textarea).parents('.rte-zone')).on('mousedown',function(e) {
...
//spellchecker attribute sc_dojoType
	...
	try {
		...
		jQuery(self.iframe_doc).on('focus',function() { self.iframe_doc.designMode(); } );
	}
	...
	jQuery(self.iframe).on('blur',function(){		//fix for IE11
		self.update_hidden_field();
	});
		
	jQuery(self.iframe_doc).on('blur',function(){
		self.update_hidden_field();
	});
	
	jQuery(self.iframe_doc.body).on('blur',function(){
		self.update_hidden_field();
	});
	
	jQuery(self.iframe_doc).on('paste', function(e){
	...
	jQuery(self.iframe).parents('form').on('submit', 
		function() { self.disable_design_mode(true); }
	);

	jQuery(self.iframe_doc).on('mouseup',function(event) { 
	...
	jQuery(self.iframe_doc).on('blur',function(event){ 
		if(self.iframe_doc.selection) 
			self.range = self.iframe_doc.selection.createRange(); // same fix for IE as above
	});

	jQuery(self.iframe_doc).on('keyup',function(event) { self.set_selected_controls( self.get_selected_element(), self.controls.rte); });
...
lwRTE.prototype.create_toolbar = function(controls) {
	...
	jQuery('.enable', tb).on('click',function() {
		self.enable_design_mode();
		return false; 
	});

	jQuery('.disable', tb).on('click',function() {
