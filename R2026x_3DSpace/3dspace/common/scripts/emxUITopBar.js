	...
	loadTopBar:function(){
		...
		TopFrameInit) {
		    ...
		    var callback = thisCommandProvider.callback = function (menuItem) {
				...
				let searchContainerCloseButton = document.querySelector('#searchContainer:not([style*="display: none"]) #closeWidget');
				if(menuItem.get('menuType')==="add" && searchContainerCloseButton){
					searchContainerCloseButton.dispatchEvent(new PointerEvent('click'));
				}
            ...
            var topbarMyHomeComplete = function (data) {
				...
				TopBar.MainMenuBar.get('home').set('onExecute', function () { 
					let searchContainerCloseButton = document.querySelector('#searchContainer:not([style*="display: none"]) #closeWidget');
					if(searchContainerCloseButton){
						searchContainerCloseButton.dispatchEvent(new PointerEvent('click'));
					}
					javascript:emxUICore.link(homedatasingle.url, homedatasingle.targetLocation); 
				}) ;
            };
            ...
            jQuery.ajax({
    		   ...
    		   dataType: 'json',
               cache:false
    		}).done(topbarGlobalMenuComplete);
            
			var toppos = jQuery("div#ExtpageHeadDiv")[0].hasChildNodes() ? (jQuery('#topbar').height() || 0) + (jQuery('#navBar').height() || 0) + (jQuery('#ExtpageHeadDiv').height() || 0):(jQuery('#topbar').height() || 0)+(jQuery('#navBar').height() || 0);
