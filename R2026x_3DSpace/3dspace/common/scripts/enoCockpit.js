...
var EnoCockPit=(function(){
	...
	loadCockpit = function(cockpitType){
		appendToWidgetCockpit(jQuery("<td></td>").append(jQuery("<div class=\"title link hidden\"><img class=\"hide\" src=\"../common/images/utilPanelToggleArrow.png\" />"+ emxUIConstants.HIDEPANEL+"</div>").on('click',function(){EnoCockPit.hidePanel();})));
	...
	hidePanel = function() {
		...
		jQuery("#right").fadeOut('200', function() {
...
//		}	
	...
	registerEvents = function(){
		jQuery("#middle").on('click',showPanel);
		attachResizeEvent();
		resizeDetailsView();
	};
	attachResizeEvent= function(){
		jQuery(window).on('resize',resizeDetailsView);	
		jQuery("#frameTable").on('load',resizeDetailsView);
	};
	resizeDetailsView = function(){
		var widthLeft = jQuery(window).width() - (jQuery("#middle").width() || 0) - (jQuery("#right").width() || 0) - 1;
	...
	drawHeader = function(header){
		...
		var rightHeaderDiv = jQuery("<div class=\"title link rightHeader\">"+header.headerString+"</div>").on('click',resetFilters);
		td.append(jQuery("<div class=\"title link \"><img class=\"hide\" src=\"../common/images/utilPanelToggleArrow.png\" />"+ emxUIConstants.HIDEPANEL+"</div>").on('click',function(){EnoCockPit.hidePanel();}));
	...
	drawWidget = function(widget){
		...
		td.append(jQuery("<div class='header "+widget.view+"' id='divHead"+widget.name+"'> "+widget.label+"</div>").on('click',function(e){toggleChartFilter(e);}));
