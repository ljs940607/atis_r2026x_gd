...
function showSlideInDialog(url, isModal, openerFrame, direction,slideinWidth){
    ...
    slideIn.css('top', (jQuery('#pageHeadDiv').height() || 0) + 'px');
    slideIn.dir = dir;
...
function closeSlideInPanel(obj) {
    ...
    if(typeof ds != "undefined" && ds.isCompassOpen){
		...
		cntpos = (jQuery("div#leftPanelMenu").width() || 0) + (jQuery("div#panelToggle").width() || 0) + 'px';
	}else if(parseInt(jQuery("div#mydeskpanel").css('left')) < 0){
		cntpos = (jQuery("div#panelToggle").width() || 0) + 'px';
	}
...
function showWindowShadeDialog(url, isModal,showOnlyDiv, genericShade){
	...
	var closeHeight = (jQuery("div#" + divName).height() || 0) + "px";
	...
	jQuery("div#" + divName).css('top', ((jQuery('#pageHeadDiv').height() || 0) - (jQuery('div#tabBar').height() || 0)) + 'px');
	jQuery('div.group-center')[0].className ="toolbar group-center";
}
...
function addMaskToGlobalToolbar(genericShade){
	if(isWindowShadeOpened() || isChangeSCDialogOpen()){
		...
		if(genericShade){
			...
			jQuery("div#layerOverlay").css('top', (jQuery('div#globalToolbar').height() || 0) + 'px');
		}
	}
}
...
function closeSecurityContextDialog()
{
	...
	var scOverlayExists = document.querySelector('#scSearchOverlay');
	if(scOverlayExists){
		scOverlayExists.style.display = "none";
	}
...
function changeSecurityContextDialog(collabSpace)
{
	...
	var isSearchContainerOpen = document.querySelector('#searchContainer:not([style*="display: none"]');
	var scOverlayExists = document.querySelector('#scSearchOverlay');
	if(isSearchContainerOpen && !scOverlayExists){
		var SCSearchOverlay = UWA.createElement('div', {
								'id': 'scSearchOverlay',
								'class': '',
								'styles':{
										'z-index': isSearchContainerOpen.style.zIndex != undefined ? String(parseInt(isSearchContainerOpen.style.zIndex) + 200): '1500',
										'display': 'block',
										'bottom': '0px',  
		...
		SCSearchOverlay.inject(document.body);
	}
	else if (isSearchContainerOpen && scOverlayExists){
		scOverlayExists.style.display = "block";
	}
