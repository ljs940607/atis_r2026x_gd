...
%>
		<tr>
			<td class="label">
				...
				int indexOfPara = report.indexOf('\n');
				int indexOfTab = report.indexOf('\t');
				int index=0;
				
				if(indexOfPara == -1 && indexOfTab == -1)
				{
				...
				else
				{
					...
					while ( indexOfPara != -1 || indexOfTab != -1)
					if ( (indexOfPara < indexOfTab && indexOfPara !=-1) || indexOfTab==-1)
					{
						String toDisplay = report.substring(index, indexOfPara-1);
						out.println(XSSUtil.encodeForHTML(context,toDisplay+"\n"));
					    %>
						<br/>
						<%
						index= indexOfPara;
						indexOfPara = report.indexOf('\n', index +1);
						indexOfTab = report.indexOf('\t', index +1);
	
	                }
					else if (indexOfTab!=-1)
					{
					    String toDisplay = report.substring(index, indexOfTab-1);
					    out.println(XSSUtil.encodeForHTML(context,toDisplay+"\n"));
					    %>
						&nbsp;&nbsp;&nbsp;&nbsp;
						<%
					...
					if(index < reportMessages.size()-1) 
					{
						String toDisplay = report.substring(index, reportMessages.size()-1);
						out.println(XSSUtil.encodeForHTML(context,toDisplay+"\n"));
					}
				}
%>
