...
<%
	...
	String contentPagelsDialog=emxGetParameter(request,"contentPagelsDialog");
	...
	boolean concurrentSimul = false;
    String contentURL = "emxSynchronizeReportDialogFS.jsp";

	  //LUS - Begin 6/23/2008 for Bug: A0627779 
	  // add these parameters to each content URL, and any others the App needs
	  contentURL += "?titleKey=" + "emxVPLMSynchro.Synchronization.Command.SyncWithVPLM.Title";
	  contentURL += "&objectId=" + XSSUtil.encodeForURL(context, objectId);
	  contentURL += "&parentOID=" + XSSUtil.encodeForURL(context, objectId);
	  contentURL += "&vplmContext=" + XSSUtil.encodeForURL(context, vplmContext) ;
	  contentURL += "&"+VPLMIntegrationConstants.SYNC_AND_TRANSFER+"=" + XSSUtil.encodeForURL(context, SYNC_AND_TRANSFER);
	  contentURL += "&"+VPLMIntegrationConstants.SYNC_DEPTH+"=" + XSSUtil.encodeForURL(context, SYNC_DEPTH) ;
	  contentURL += "&"+VPLMIntegrationReporter.DETAILED_REPORT+"=" + XSSUtil.encodeForURL(context, DETAILED_REPORT);
	  contentURL += "&SYNC_FORWARD=" + XSSUtil.encodeForURL(context, SYNC_FORWARD);
	  contentURL += "&targetLocation="+XSSUtil.encodeForURL(context, targetLocation);
	  contentURL += "&"+VPLMIntegrationConstants.MAPKEY_SIMUL+"=" +"COLLAB_AFTER_SIMUL";
...
<script language="javascript">
		...
		function doUnload()
		{
		 ...
		 function executeAfterSimulation(){
			 	<%-- if(<%=concurrentSimul%>){
			 		<%=contentURL += "&"+VPLMIntegrationConstants.MAPKEY_SIMUL+"=" +"SIMUL_BY_OTHERS"%>;
			 	}else{
			 		<%=contentURL += "&"+VPLMIntegrationConstants.MAPKEY_SIMUL+"=" +"COLLAB_AFTER_SIMUL"%>;
			 	}
			 	 --%>
