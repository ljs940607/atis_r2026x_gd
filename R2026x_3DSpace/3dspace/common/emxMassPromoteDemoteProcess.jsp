...
<%    
        ...
        else
        {
            ...
            if(!tempIdList.isEmpty())
            {
                ...
                List<String> doObjList = new ArrayList<>();
    	        for(int i=0;i < tempIdList.size();i++)
    	        {   	     
    	            String objectId= tempIdList.get(i);
    	            try{
    	        ...
    	        if(!doObjList.isEmpty()) {
                	Map<String, String> idsAndErrorMsg = com.matrixone.apps.framework.lifecycle.LifeCycleUtil.checkAndCreateRouteOnObjects(context, doObjList); 
                	for (Map.Entry<String, String> entry : idsAndErrorMsg.entrySet()) {
                        System.out.println(entry.getKey() + " : " + entry.getValue());
                        errorMap = new HashMap();
                        errorMap.put("id",entry.getKey());
                        errorMap.put("error",entry.getValue());
                        errorList.add(errorMap);
                        tempIdList.remove(entry.getKey()); 
                    }
                }
            }
        ...
        if(finalMap!= null && finalMap.size() > 0)
        {
            ...
            for(int i=0;i < valueMapList.size();i++)
            {
                ...
                { 
                    try
                    {
                    	com.matrixone.apps.framework.lifecycle.LifeCycleUtil.checkActiveRoutes(context, dObject);
