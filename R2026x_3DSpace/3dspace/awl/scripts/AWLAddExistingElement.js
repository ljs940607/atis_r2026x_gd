...
(function($){
	POA_Edit_Instance_connector = new function () {
		
		function setupQt()
		{
			 new QWebChannel(qt.webChannelTransport, function (channel) {
				 window.Q = channel.objects.addexistingelementhelper; 			
				});
		}
		setupQt();
	...
	this.addCEToPOAForConnector = function(node, mcIDs) {
		POA_Edit_Instance.addCEToPOAForConnector(node, mcIDs);
	};
	   
		 	   //On Press of OK button
	 this.refreshArtworkAssembly = function(){
			 //alert('Ok - Refresh assembly and close');
			 Q.emitRefreshSignal();
		};
		//On Press of Close Button
		 this.closeAddExistingDialog = function(){
			 //Q.refreshArtworkAssembly();
			 Q.closeAddExistingElementDialog();
		 };
	...
	this.localCopyCellClickHandler=function(e){
		POA_Edit_Instance.localCopyCellClickHandler(e);
	};
  this.addExistingCLHandler=function(e){
    POA_Edit_Instance.addExistingCLHandler(e);
  };
	/*
	...
	*/
	...
	this.disableClearButton = function(){
		if($("td.success").length === 0 && $("th.POASelected").length === 0 && $("td div.selected").length === 0){
			...
			customEditUtil.hideToolTip('#clear');
		}
	};
...
this.createTableWithHeader = function(poaIds) {
	...
	// close button on connector-t26
	var tfootHTML = $("<tfoot>").attr("id","editTableFooter");
	var tfootTR = $("<tr>");
	var tfootTD = $("<td>");
	var tfootCancelButton =  $('<button/>', {
        text: POA_EDIT_LABELS['emxCommonButton.Close'],
        id: "footerButtons",
        click: editInstance.closeAddExistingDialog
    });
	tfootTD.append(tfootCancelButton);
	tfootTR.append(tfootTD);
	tfootHTML.append(tfootTR);
...
//BNN2 Sorting Logic - Ends
	...
	//display name
	var displayNameHTML = $("<th/>").addClass("edit-table-header display-name-header").html(POA_EDIT_LABELS['emxFramework.Basic.State']+ "</br>"+POA_EDIT_LABELS['AWL.Common.Revision']+"</br>")// RRR1 Use New Label html(POA_EDIT_LABELS['emxAWL.Label.POAEdit.DisplayName']);
	...
	$.each(poaIdArr , function (index){
		...
		var poaNameState = poaInfoMap.Name+" ("+poaState+")";								//editInstance.getDisplayName(poaInfoMap.Name)+" ("+poaState+")";//RRR1
...
this.filterPOAEditPage = function (){
  ...
  //  var selectedOption = $('#filterSelection option:selected').html();
    ...
    $("tbody tr").filter(function(){
    	...
    	var displayName = $(this).children('td.elementtype').attr('dispName');
...
this.addMCA_POACell = function (isGraphic,copyId,poaId,localCopyInfo,eachNewCell,isInline){
		...
		if($(lc_lang_Info).length > 0)
		{
			...
			if(copyListArray.join(",").length >15)
			{
					...
					copylistImagespan.tooltip();
					copyListSpan.attr("title",copyListConnected);
					copyListSpan.tooltip();
			} else {
	...
	this.populateTable = function (poaIds){
		...
		//Get 1st POA ID
		if(typeof poaIDNotToBeRemoved === null)
			poaIDNotToBeRemoved = poaIds;
	};
...
this.handleHeaderScrollBar = function(){
	...
	this.resequenceCellInformation = function(cell,languageNamesOnHeaderArr,languageIdsOnHeaderArr, localCopyInfo){
		POA_Edit_Instance.resequenceCellInformation(cell,languageNamesOnHeaderArr,languageIdsOnHeaderArr, localCopyInfo);
	};
	...
	this.createCommonToolbar = function (element) {
		...
		var commonMenus = [ {id:"clear", label:POA_EDIT_LABELS["emxCommonButton.ClearSelection"]},
            { id: "refreshAssembly", label: POA_EDIT_LABELS["emxAWL.Label.AssemblyElementSync"]}
			  ];
	...
	this.getToolbarCommand = function(id,title){
		'use strict';
		var span = $("<span>").addClass("btn btn-default poa-edit-toolbar").attr("style","background-color:transparent !important;");
		//var asModButton= $('<image/>');
		span.attr("id",id);
		span.attr("title", title);
		span.tooltip();
		span.attr("disabled","disabled");
		span.attr("class","btn btn-default disabled poa-edit-toolbar "+ id);
	...
	this.selectAllHeaderHandler = function(){
		...
		var commandsArray = ['addArtworkMaster','createMasterCopyElement','createGraphicElement','manageCountryLanguages','removePOA','removeCopyList','connectComprisedPOA', 'disconnectComprisedPOA','addElementFromComprisedPOA','resequenceElements','saveSequenceNumber','addExistingElem','refreshAssembly', 'addExistingCL'];
	...
	//If callbackFunction passed as string then declaration should be global and without namespaces.
	...
	//Overriden to show Local Copy cell values after adding artwork Master in POA Edit page
