	...
	$.ajax({
		...
		success: function(response){
			if(response.result=="success") {
				...
				Q.showJavaScriptAlert(labelObj['emxAWL.Message.NoObjectsFound']);//IR-581111 - Uniform alert box
			}
		}
	});
}
...
function addRow(row)
{
	...
	var associateImage = $('<img>').attr('src', 'connector/images/Associate.png').attr("class","associate-icon").attr("onClick","associateButtonClicked(event)").attr("title",labelObj['emxAWL.Common.AssociateTemplate']);
	//$(image).tooltip(); 
