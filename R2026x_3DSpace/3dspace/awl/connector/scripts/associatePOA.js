...
function hasFile(downloadFileURL) {
	$.ajax({
		url: downloadFileURL,
		dataType: "json",
		success: function(response){
...
function addRow(row)
{
	...
	var associateImage = $('<img>').attr('src', 'connector/images/Associate.png').attr("class","associate-icon").attr("onClick","associateButtonClicked(event)").attr("title",labelObj['emxAWL.ToolTip.AssociatePOA']);
	//$(image).tooltip(); 
