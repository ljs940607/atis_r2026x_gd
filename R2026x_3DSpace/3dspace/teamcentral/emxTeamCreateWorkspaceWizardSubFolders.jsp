...
<script language="javascript">
 ...
 <%
            String personUrl = "../common/emxIndentedTable.jsp?program=emxDomainAccess:getObjectAccessList&table=DomainAccess&editLink=true&selection=multiple&toolbar=DomainAccessToolBar&header=emxFramework.WorkSpaceAccessMembersDialog.AccessMembers&editRootNode=false&objectBased=false&showClipboard=false&displayView=details&showPageURLIcon=false&massUpdate=false&HelpMarker=emxhelpmultipleownershipaccess&suiteKey=Framework&objectId="+strObjectId+"&template="+ XSSUtil.encodeForURL(context, template) +"&templateId="+XSSUtil.encodeForURL(context, templateId) +"&submitLabel=emxFramework.Button.Submit";
  ...
  function goBack(){
	...
	<div id='divPageBody'>
       	<div id="divFolderTreePabe">
       		<iframe name="folderTree" id="folderTree" src="emxTeamFoldersView.jsp?jsTreeID=<%=XSSUtil.encodeForURL(context, jsTreeID)%>&objectId=<%=XSSUtil.encodeForURL(context, strObjectId)%>&template=<%=XSSUtil.encodeForURL(context, template)%>&templateId=<%=XSSUtil.encodeForURL(context, templateId)%>" allowtransparency="true" frameborder="0"></iframe>
        </div>
