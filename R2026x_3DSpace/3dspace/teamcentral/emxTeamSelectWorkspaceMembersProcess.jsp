...
<%
  ...
  if(changedAccess!= null && !"".equals(changedAccess))
  {
      ...
      session.setAttribute("routeMemberMapList",routeMemberMapList);
 %>
...
<%
  ...
  if(sRouteId != null && (!sRouteId.equals(""))){
   ...
   if("Person".equalsIgnoreCase(memberType)){
    ...
    if(roleList != null && roleList.size() > 0){
      try{
        routeObject.addRoleAsRouteMember(context, roleList , allowExtend);
         ...
         session.setAttribute("error.message", i18nNow.getI18nString(e.getMessage(), "emxTeamCentralStringResource" ,sLanguage));
      }
    }
    ...
    if(!sErrorMsg.equals("")){
      session.setAttribute("error.message", sErrorMsg);
    }
  ...
  MapList memberMapList =(MapList)session.getAttribute("routeMemberMapList");
  ...
  MapList roleMapList =(MapList)session.getAttribute("routeRoleMapList");
...
if(roleList.size() > 0)
{
  ...
  session.setAttribute("taskMapList",taskMapList);
  session.setAttribute("routeMemberMapList",memberMapList);
  session.setAttribute("routeRoleMapList",roleMapList);
