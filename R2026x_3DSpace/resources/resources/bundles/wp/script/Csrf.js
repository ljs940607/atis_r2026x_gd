...
define('ds/Csrf',
        ...
        */
        ...
        function isValidBaseUrl (url) {
            return (options.domainBaseUrlStrict === true) ? url.indexOf(window.dsDashboardWindow.parent.dsBaseUrl) === 0 :  true;
        }
        ...
        function isValidUrl(url) {
            ...
            if ((parsedUrl.protocol === 'http') || (parsedUrl.protocol === 'https')) {
                // will push token if:
                result = isValidDomain(parsedUrl.domain) // [tgt&src domain are the same]
                         && isValidPort(parsedUrl.port, parsedUrl.protocol) // [tgt&src port are the same]
                         // ([tgt&src basename are same, ie: onprem3dx/dashboard vs onprem3dx/passport] OR (tgt is dashboard window url))
                         && (isValidBaseUrl(url) || (url.indexOf(window.dsDashboardWindow.topPlatformWindow.dsBaseUrl)==0));
        ...
        function start() {
            ...
            try {
                if (typeof dsCsrfOptions === 'undefined') {
                    options = window.dsDashboardWindow.parent.dsCsrfOptions;
                } else {
